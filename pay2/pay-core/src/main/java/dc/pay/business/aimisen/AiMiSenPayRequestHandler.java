package dc.pay.business.aimisen;
...
@RequestPayHandler("AIMISEN")
public final class AiMiSenPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(AiMiSenPayRequestHandler.class);
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        if(!channelWrapper.getAPI_MEMBERID().contains("&")){
            throw new PayException("商户号格式错误请使用&符号链接mchid和src_code如： mchid&src_code");
        }
        ...
        log.debug("[艾米森]-[请求支付]-1.组装请求参数完成：" + JSON.toJSONString(payParam));
        return payParam;
    }
    ...
    protected String buildPaySign(Map payParam) throws PayException {
        ...
        log.debug("[艾米森]-[请求支付]-2.生成加密URL签名完成：" + JSON.toJSONString(pay_md5sign));
        return pay_md5sign;
    }
    protected List<Map<String, String>> sendRequestGetResult(Map<String, String> payParam, String pay_md5sign) throws PayException {
        ...
        try {
            ...
            if (!respcd.equalsIgnoreCase("0000")) {
                log.error("[艾米森]3.发送支付请求，及获取支付请求结果：" + resJson.toJSONString() + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
                throw new PayException(resJson.toJSONString());
            }

                if (api_channel_bank_name.endsWith("WX_SM") || api_channel_bank_name.endsWith("ZFB_SM")  ||api_channel_bank_name.endsWith("QQ_SM")||api_channel_bank_name.endsWith("ZFB_FS")  ) {
            ...
            log.error("[艾米森]3.发送支付请求，及获取支付请求结果出错：", e);
            throw new PayException(e.getMessage(), e);
        }
        log.debug("[艾米森]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：" + JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[艾米森]-[请求支付]-4.处理请求响应成功：" + JSON.toJSONString(requestPayResult));
