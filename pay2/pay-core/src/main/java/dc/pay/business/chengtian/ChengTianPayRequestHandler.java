package dc.pay.business.chengtian;
...
@RequestPayHandler("CHENGTIANZHIFU")
public final class ChengTianPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(ChengTianPayRequestHandler.class);
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        ...
        log.debug("[诚天支付]-[请求支付]-1.组装请求参数完成：" + JSON.toJSONString(payParam));
        return payParam;
    }
    ...
    protected String buildPaySign(Map<String,String> params) throws PayException {
        ...
        log.debug("[诚天支付]-[请求支付]-2.生成加密URL签名完成：" + JSON.toJSONString(pay_md5sign));
        return pay_md5sign;
    }
        ...
        try {
            if (HandlerUtil.isWY(channelWrapper) || HandlerUtil.isYLKJ(channelWrapper)  || HandlerUtil.isWapOrApp(channelWrapper)    ) {
                ...
                if(StringUtils.isNotBlank(resultStr)){
                    ...
                    Element bodyEl = document.select("div #code").first();
                      String  imgSrc="";
                      try{
                          imgSrc =QRCodeUtil.decodeByUrl(bodyEl.getElementsByTag("img").first().attr("src"));
                      }catch (Exception e){
                          throw new PayException("解析第三方二维码失败："+resultStr);
                      }

                    if(StringUtils.isNotBlank(imgSrc)){
                        result.put(QRCONTEXT, HandlerUtil.UrlDecode(imgSrc));
                        payResultList.add(result);
                    }else{
                        throw new PayException("解析第三方二维码失败："+resultStr);
                    }
...
//                    }
             ...
             log.error("[诚天支付]3.发送支付请求，及获取支付请求结果出错：", e);
             throw new PayException(e.getMessage(), e);
        }
        log.debug("[诚天支付]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：{}" ,JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[诚天支付]-[请求支付]-4.处理请求响应成功：" + JSON.toJSONString(requestPayResult));
