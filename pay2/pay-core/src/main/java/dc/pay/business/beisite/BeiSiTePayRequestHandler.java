package dc.pay.business.beisite;
 ...
 * Sep 5, 2019
 */
@RequestPayHandler("BEISITE")
public final class BeiSiTePayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(BeiSiTePayRequestHandler.class);

    //参数名称    参数含义    是否必填    参与签名    参数说明
    //mch_id  商户号 是   是   平台分配商户号
    private static final String mch_id                ="mch_id";
    //out_order_id    订单号 是   是   上送订单号唯一, 字符长度20
    private static final String out_order_id                ="out_order_id";
    //type    支付类型    是   是   参考后续说明
    private static final String type                ="type";
    //notifyurl   服务端通知   是   是   服务端返回地址.（POST返回数据）
    private static final String notifyurl                ="notifyurl";
    //returnurl   页面跳转    是   是   页面跳转返回地址
    private static final String returnurl                ="returnurl";
    //price   订单金额    是   是   商品金额(元)
    private static final String price                ="price";
    //extend  附加字段    否   否   此字段在返回时按原样返回
...
//            log.error("[贝丝特]-[请求支付]-“支付通道商号”输入数据格式为【中间使用&分隔】：商户号&机构号" );
//            throw new PayException("[贝丝特]-[请求支付]-“支付通道商号”输入数据格式为【中间使用&分隔】：商户号&机构号" );
//        }
        ...
        log.debug("[贝丝特]-[请求支付]-1.组装请求参数完成：{}" ,JSON.toJSONString(payParam));
        return payParam;
    }
     ...
     protected String buildPaySign(Map<String,String> api_response_params) throws PayException {
        ...
        log.debug("[贝丝特]-[请求支付]-2.生成加密URL签名完成：{}" ,JSON.toJSONString(signMd5));
        return signMd5;
    }
...
//            String resultStr = RestTemplateUtil.sendByRestTemplateRedirect(channelWrapper.getAPI_CHANNEL_BANK_URL(), payParam, String.class, HttpMethod.POST);
            ...
            //    log.error("[贝丝特]-[请求支付]-3.1.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
            //    throw new PayException(resultStr);
            //    //log.error("[贝丝特]-[请求支付]-3.1.发送支付请求，获取支付请求返回值异常:返回空,参数：{}",JSON.toJSONString(map));
...
//            if (null != jsonObject && jsonObject.containsKey("code") && "1".equalsIgnoreCase(jsonObject.getString("code"))  && jsonObject.containsKey("codeimg") && StringUtils.isNotBlank(jsonObject.getString("codeimg"))) {
        ...
        log.debug("[贝丝特]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：{}",JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[贝丝特]-[请求支付]-4.处理请求响应成功：{}",JSON.toJSONString(requestPayResult));
