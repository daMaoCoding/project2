package dc.pay.business.aitaozhifu;
...
@RequestPayHandler("AITONGZHIFU")
public final class AiTaoZhiFuPayRequestHandler extends PayRequestHandler {
        private static final Logger log = LoggerFactory.getLogger(AiTaoZhiFuPayRequestHandler.class);
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        ...
        log.debug("[爱淘支付]-[请求支付]-1.组装请求参数完成：" + JSON.toJSONString(payParam));
        return payParam;
    }
    ...
    protected String buildPaySign(Map<String,String> params) throws PayException {
        ...
        log.debug("[爱淘支付]-[请求支付]-2.生成加密URL签名完成：" + JSON.toJSONString(pay_md5signA));
        return pay_md5signA;
    }
        ...
        try {
            if (HandlerUtil.isWY(channelWrapper) || HandlerUtil.isYLKJ(channelWrapper) || channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG().equalsIgnoreCase("904") ) { //对接时候只能使用微信，支付宝wap(返回html)
                ...
                if(StringUtils.isNotBlank(resultStr)){
                    Elements htmlEls = Jsoup.parse(resultStr).select("div>img");
                    if(htmlEls.size()==3){
                        String imgSrc= htmlEls.eq(1).attr("src");
                        if(StringUtils.isNotBlank(imgSrc)){
                            imgSrc = "http://payaitao.com".concat(imgSrc);
                            String qrContent = QRCodeUtil.decodeByUrl(imgSrc);
             ...
             log.error("[爱淘支付]3.发送支付请求，及获取支付请求结果出错：", e);
             throw new PayException(e.getMessage(), e);
        }
        log.debug("[爱淘支付]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：{}" ,JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[爱淘支付]-[请求支付]-4.处理请求响应成功：" + JSON.toJSONString(requestPayResult));
