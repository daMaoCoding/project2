...
@RequestPayHandler("YUNBAO")
public final class YunBaoPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(YunBaoPayRequestHandler.class);
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        ...
        log.debug("[云宝]-[请求支付]-1.组装请求参数完成：" + JSON.toJSONString(payParam));
        return payParam;
    }
    ...
    protected String buildPaySign(Map payParam) throws PayException {
        ...
        String pay_md5sign = YunBaoUtil.Md5(signStr + "|" + channelWrapper.getAPI_KEY());
        // pay_md5sign = HandlerUtil.getMD5UpperCase(paramsStr).toLowerCase();
        log.debug("[云宝]-[请求支付]-2.生成加密URL签名完成：" + JSON.toJSONString(pay_md5sign));
        return pay_md5sign;
    }
    ...
    protected List<Map<String, String>> sendRequestGetResult(Map<String, String> payParam, String pay_md5sign) throws PayException {
        ...
        try {
            ...
            if((HandlerUtil.isYLKJ(channelWrapper) ||HandlerUtil.isWapOrApp(channelWrapper) || HandlerUtil.isWebJdKjzf(channelWrapper)) && !channelWrapper.getAPI_CHANNEL_BANK_NAME().contains("QQ")){
                ...
                if (firstPayresult.length() < 10) {
                    log.error("[云宝]3.发送支付请求，及获取支付请求结果：" + firstPayresult + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
                ...
                if (resJSON.containsKey("status") && resJSON.getString("status").equals("0") && resJSON.containsKey("code_url") && StringUtils.isNotBlank(resJSON.getString("code_url"))) {
                    ...
                    HashMap<String, String> result = Maps.newHashMap();
                    if (channelWrapper.getAPI_CHANNEL_BANK_NAME().contains("_WAP_QQ_")) {
                    	result.put(HTMLCONTEXT,  HandlerUtil.getHtmlContent(URLDecoder.decode(wxQRContext, "UTF-8"),new HashMap<>()).toString());
					}else {
						result.put(QRCONTEXT,  URLDecoder.decode(wxQRContext, "UTF-8"));
            ...
            log.error("[云宝]3.发送支付请求，及获取支付请求结果出错：", e);
            throw new PayException(e.getMessage(), e);
        }
        log.debug("[云宝]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：" + JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[云宝]-[请求支付]-4.处理请求支付成功：" + JSON.toJSONString(requestPayResult));
