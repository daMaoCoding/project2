 ...
 * Dec 18, 2018
 */
@RequestPayHandler("CHANGLIANZHIFU")
public final class ChangLianZhiFuPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(ChangLianZhiFuPayRequestHandler.class);
...
//    pay_version		        系统接口版本			是					否	
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        ...
        log.debug("[长联支付]-[请求支付]-1.组装请求参数完成：{}" ,JSON.toJSONString(payParam));
        return payParam;
    }

     protected String buildPaySign(Map<String,String> api_response_params) throws PayException {
        String signSrc=String.format("%s%s%s%s%s%s",
        		pay_memberid+"="+api_response_params.get(pay_memberid)+"&",
        		pay_bankcode+"="+api_response_params.get(pay_bankcode)+"&",
        		pay_amount+"="+api_response_params.get(pay_amount)+"&",
        		pay_orderid+"="+api_response_params.get(pay_orderid)+"&",
        		pay_notifyurl+"="+api_response_params.get(pay_notifyurl),
        		channelWrapper.getAPI_KEY()
        );
        String paramsStr = signSrc.toString();
        String signMD5 = HandlerUtil.getMD5UpperCase(paramsStr).toLowerCase();
        log.debug("[长联支付]-[请求支付]-2.生成加密URL签名完成：{}" ,JSON.toJSONString(signMD5));
        return signMD5;
    }
    ...
    protected List<Map<String, String>> sendRequestGetResult(Map<String, String> payParam, String pay_md5sign) throws PayException {
        ...
        log.debug("[长联支付]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：{}",JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[长联支付]-[请求支付]-4.处理请求响应成功：{}",JSON.toJSONString(requestPayResult));
