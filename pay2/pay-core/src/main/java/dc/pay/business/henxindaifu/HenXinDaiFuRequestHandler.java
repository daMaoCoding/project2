 ...
 * 05 16, 2019
 */
@RequestDaifuHandler("HENXINDAIFU")
public final class HenXinDaiFuRequestHandler extends DaifuRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(HenXinDaiFuRequestHandler.class);
    ...
    protected PayEumeration.DAIFU_RESULT requestDaifuAllInOne(Map<String,String> payParam,Map<String,String> details) throws PayException {
        try {
          ...
          //生成md5
            ...
            signSrc.append(key+"="+channelWrapper.getAPI_PUBLIC_KEY().split("&")[0]);
            ...
            postMap.put("merId", channelWrapper.getAPI_MEMBERID());
            postMap.put("version", "1.1");
            String dataEncStr= SecurityUtils.encrypt(JSON.toJSONString(payParam), channelWrapper.getAPI_PUBLIC_KEY().split("&")[1]);
   ...
   // 其他情况抛异常
    ...
    protected PayEumeration.DAIFU_RESULT queryDaifuAllInOne(Map<String, String> payParam,Map<String, String> details) throws PayException {
       if(1==2) throw new PayException("[亨鑫代付][代付][查询订单状态]该功能未完成。");
        try {
            ...
            details.put(RESPONSEKEY, resultStr);//强制必须保存下第三方结果
            if(StringUtils.isNotBlank(resultStr) ){
                return getDaifuResult(responseJsonObject.getString("data"),true);
            }else{ throw new PayException(EMPTYRESPONSE);}
    ...
    protected long queryDaifuBalanceAllInOne(Map<String,String> payParam,Map<String,String> details) throws PayException {
        ...
        try {
            ...
            log.error("[亨鑫代付][代付余额查询]出错,错误消息：{},参数：{}",e.getMessage(),JSON.toJSONString(payParam),e);
...
//          请求状态(成功:200 失败：1)
        ...
        if(HandlerUtil.valJsonObj(jsonObj, "code", "200")){
   	         ...
   	         if (!HandlerUtil.valJsonObj(data, "orderState","1","2","3")) return PayEumeration.DAIFU_RESULT.UNKNOW;
