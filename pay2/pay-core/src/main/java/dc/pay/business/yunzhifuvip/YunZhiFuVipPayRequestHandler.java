 ...
 * Mar 11, 2019
 */
@RequestPayHandler("YUNZHIFUVIPZHIFU")
public final class YunZhiFuVipPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(YunZhiFuVipPayRequestHandler.class);
...
// post_url    通知接口地址    string    必填    用户支付成功后，云支付VIP服务器会POST调用这个链接，并携带一系统参数，具体参数请查看付款成功回调接口。
...
// key    MD5验签    string    必填    此字段为uid+auth_code+money+channel+post_url+return_url+order_id+order_uid+goods_name进行字符串拼接后再进行md5-32位加密后的值。伪代码示例：md5(uid+auth_code+money+channel+post_url+return_url+order_id+order_uid+goods_name)
    private static final String uid                ="uid"; // 用户ID-商户号
    private static final String money                 ="money"; // 金额 单位：元，
    private static final String channel                ="channel"; // 渠道  支付宝-参数:alipay
    private static final String post_url                ="post_url"; // 通知接口地址
    private static final String return_url              ="return_url"; // 跳转地址
    private static final String order_id              ="order_id"; // 订单号
    private static final String order_uid             ="order_uid"; // 用户编号
    private static final String goods_name             ="goods_name"; // 商品名
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        ...
        log.debug("[云支付Vip]-[请求支付]-1.组装请求参数完成：{}" ,JSON.toJSONString(payParam));
        return payParam;
    }

    protected String buildPaySign(Map<String,String> api_response_params) throws PayException {
        //此字段为uid+auth_code+money+channel+post_url+return_url+order_id+order_uid+goods_name进行字符串拼接后再进行md5-32位加密后的值。
        ...
        signSrc.append(api_response_params.get(channel));
        signSrc.append(api_response_params.get(post_url));
        ...
        log.debug("[云支付Vip]-[请求支付]-2.生成加密URL签名完成：{}" ,JSON.toJSONString(signMd5));
        return signMd5;
    }
    ...
    protected List<Map<String, String>> sendRequestGetResult(Map<String, String> payParam, String pay_md5sign) throws PayException {
        ...
        try {
                ...
                try {
                    ...
                    e.printStackTrace();
                    log.error("[云支付Vip]-[请求支付]-3.1.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
                ...
                if (null != jsonObject && jsonObject.containsKey("code") && "200".equalsIgnoreCase(jsonObject.getString("code"))
                    ...
                    log.error("[云支付Vip]-[请求支付]-3.2.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
                    throw new PayException(resultStr);
                }
            
        } catch (Exception e) {
            log.error("[云支付Vip]-[请求支付]-3.3.发送支付请求，及获取支付请求结果出错：", e);
            throw new PayException(null != e.getMessage() ? e.getMessage() : "请求第三方，返回Null",e);
        }
        ArrayList<Map<String, String>> payResultList = Lists.newArrayList();
        payResultList.add(result);
        log.debug("[云支付Vip]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：{}",JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[云支付Vip]-[请求支付]-4.处理请求响应成功：{}",JSON.toJSONString(requestPayResult));
