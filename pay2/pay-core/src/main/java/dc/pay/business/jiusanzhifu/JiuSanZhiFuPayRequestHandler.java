...
@RequestPayHandler("JIUSANZHIFU")
public final class JiuSanZhiFuPayRequestHandler extends PayRequestHandler {
	private static final Logger log = LoggerFactory.getLogger(JiuSanZhiFuPayRequestHandler.class);
	...
	protected Map<String, String> buildPayParam() throws PayException {
		...
		log.debug("[93支付]-[请求支付]-1.组装请求参数完成：" + JSON.toJSONString(payParam));
		return payParam;
	}
	...
	protected String buildPaySign(Map<String, String> params) throws PayException {
		...
		log.debug("[93支付]-[请求支付]-2.生成加密URL签名完成：" + JSON.toJSONString(pay_md5sign));
		return pay_md5sign;
	}
		...
		try {
			if (1==1 || HandlerUtil.isWY(channelWrapper) || HandlerUtil.isYLWAP(channelWrapper)
				...
				//if(channelWrapper.getAPI_CHANNEL_BANK_NAME().equalsIgnoreCase("TIANZE_BANK_WEBWAPAPP_WX_SM")){ //如果为微信扫码
				    ...
				    if(qrImg == null){
				        String errorMsg = "[93支付]-[请求支付]3.1.发送支付请求，及获取支付请求结果出错："+html;
				        log.error(errorMsg);
				    ...
				    if(StringUtils.isBlank(qrImgUrl)){
				        String errorMsg = "[93支付]-[请求支付]3.2.未获取到二维码地址。";
				        log.error(errorMsg);
				        throw new PayException(html);
				    }
			...
			log.error("[93支付]3.2.发送支付请求，及获取支付请求结果出错：", e);
			throw new PayException(e.getMessage(), e);
		}
		
		log.debug("[93支付]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：{}", JSON.toJSONString(payResultList));
	...
	protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
		...
		log.debug("[93支付]-[请求支付]-4.处理请求响应成功：" + JSON.toJSONString(requestPayResult));
