      ...
      if (1==2&&(HandlerUtil.isWY(channelWrapper) || HandlerUtil.isYLKJ(channelWrapper)  ||  HandlerUtil.isWapOrApp(channelWrapper))) {
    	  ...
    	  String resultStr = RestTemplateUtil.postXml(channelWrapper.getAPI_CHANNEL_BANK_URL(), XmlUtil.map2Xml(payParam, false, "xml", false));
	        ...
	        if (null != xml2Map && xml2Map.containsKey("status") && "0".equals(xml2Map.get("status")) && "0".equals(xml2Map.get("result_code"))) {
	            if(HandlerUtil.isWapOrApp(channelWrapper) && StringUtils.isNotBlank(xml2Map.get("redirect_url"))){
                    String code_url = xml2Map.get("redirect_url");
                    result.put(JUMPURL, code_url);
	            }else if(HandlerUtil.isWxSM(channelWrapper) && StringUtils.isNotBlank(xml2Map.get("code_url"))){
	            ...
	            log.error("[默默支付]-[请求支付]-3.5.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
	            throw new PayException(resultStr);
	        }
	        payResultList.add(result);
      }
