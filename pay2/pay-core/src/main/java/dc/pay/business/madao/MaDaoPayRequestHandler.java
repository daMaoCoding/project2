 ...
 * Oct 30, 2018
 */
@RequestPayHandler("MADAO")
public final class MaDaoPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(MaDaoPayRequestHandler.class);

    //参数名称               参数含义                         是否必填        参数长度        参数说明        顺序
    //p0_Cmd                 业务类型                          是               Max(20)        固定值 “Buy”         1
    //p1_MerId               商户编号                          是               Max(11)        4位数字，是商户在[API支付平台]系统的唯一身份标识  请登录商户后台查看        2
    ...
    //p8_Url                 商户接收支付成功数据的地址        是               Max(200)        支付成功后[API支付平台]会向该地址分别各发送一次“服务器点对点通知（异步通知）”和“浏览器重定向（同步通知）”的成功通知，该地址不可以带参数，不建议使用过长的URL地址 参照示例如： http://www.123.com/callback.action        9
    ...
    //pr_NeedResponse        应答机制                          是               Max(1)        固定值为“1”：需要应答机制；   收到[API支付平台]服务器点对点支付成功通知（异步通知），必须回写“success”（无关大小写），即使您收到成功通知时发现该订单已经处理过，也要正确回写“success”，该通知仅发送一次，如需补发通知，请到商户后台成功订单操作重发通知        13
    //hmac                   签名数据                          是               Max(32)        产生hmac需要两个参数，并调用相关API；  参数1： STR，列表中的参数原值按照签名顺序拼接所产生的字符串，注意null要转换为 “”，并确保无乱码   参数2：商户密钥；请登录商户后台查看   各语言范例已经提供封装好了的方法用于生成此参数  如果以上两个参数有错误，则该参数必然错误，提示“签名失败”
...
//    private static final String signature  ="hmac";
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        ...
        log.debug("[码道]-[请求支付]-1.组装请求参数完成：{}" ,JSON.toJSONString(payParam));
        return payParam;
    }
    ...
    protected String buildPaySign(Map<String,String> api_response_params) throws PayException {
        ...
        log.debug("[码道]-[请求支付]-2.生成加密URL签名完成：{}" ,JSON.toJSONString(signMd5));
        return signMd5;
    }
...
//                log.error("[码道]-[请求支付]-3.1.发送支付请求，获取支付请求返回值异常:返回空,参数：{}",JSON.toJSONString(payParam));
//                throw new PayException("返回空,参数："+JSON.toJSONString(payParam));
//            }
//            System.out.println("url参数1=======》"+channelWrapper.getAPI_CHANNEL_BANK_URL());
//            System.out.println("入参数1=======》"+JSON.toJSONString(payParam));
//            System.out.println("resultStr1=====>"+resultStr);
//            
//            Document document2 = Jsoup.parse(resultStr);  //Jsoup.parseBodyFragment(html)
//            Element formEl2 = document2.getElementsByTag("form").first();
//            Map<String, String> secondPayParam = HandlerUtil.parseFormElement(formEl2);
//            String action2 = secondPayParam.get("action");
//            if (!action2.startsWith("http")) {
//                action2 = "https://madaofu.com/"+action2;
//            }
//            String resultStr2 = RestTemplateUtil.postForm(action2, secondPayParam,"UTF-8");
////            String resultStr2 = RestTemplateUtil.sendByRestTemplateRedirect(action2, secondPayParam, String.class, HttpMethod.GET);
////            String resultStr2 = RestTemplateUtil.postJson(action2, secondPayParam);
//            if (StringUtils.isBlank(resultStr2)) {
//                log.error("[码道]-[请求支付]-3.1.发送支付请求，获取支付请求返回值异常:返回空,参数：{}",JSON.toJSONString(secondPayParam));
//                throw new PayException("返回空,参数："+JSON.toJSONString(resultStr2));
//            }
//            System.out.println("url参数2=======》"+action2);
//            System.out.println("入参数2=======》"+JSON.toJSONString(secondPayParam));
...
//            Map<String, String> thirPayParam = HandlerUtil.parseFormElement(formEl3);
//            String action3 = thirPayParam.get("action");
//            if (!action3.startsWith("http")) {
////                action3 = "https://madaofu.com/pay/"+action3;
//                action3 = "https://madaofu.com/pay/getorder.aspx";
////                action3 = "https://madaofu.com/pay/aliperson/Default.aspx";
//            }
////            String resultStr3 = RestTemplateUtil.postForm(action3, thirPayParam,"UTF-8");
//            String resultStr3 = RestTemplateUtil.sendByRestTemplateRedirect(channelWrapper.getAPI_CHANNEL_BANK_URL(), payParam, String.class, HttpMethod.POST);
//            if (StringUtils.isBlank(resultStr3)) {
//                log.error("[码道]-[请求支付]-3.1.发送支付请求，获取支付请求返回值异常:返回空,参数：{}",JSON.toJSONString(thirPayParam));
//                throw new PayException("返回空,参数："+JSON.toJSONString(resultStr3));
//            }
//            System.out.println("url参数3=======》"+action3);
//            System.out.println("入参数3=======》"+JSON.toJSONString(thirPayParam));
...
//              log.error("[码道]-[请求支付]-3.3.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
...
//                e.printStackTrace();
//                log.error("[码道]-[请求支付]-3.5.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
//                throw new PayException(resultStr);
//            }
//            if (StringUtils.isBlank(decode)) {
//                log.error("[码道]-[请求支付]-3.6.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
//                throw new PayException(resultStr);
//            }
...
//        }
        ...
        log.debug("[码道]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：{}",JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[码道]-[请求支付]-4.处理请求响应成功：{}",JSON.toJSONString(requestPayResult));
