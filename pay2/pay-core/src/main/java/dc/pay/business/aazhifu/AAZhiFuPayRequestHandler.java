...
//    private static final String signature  ="signature";
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        ...
        if (null == aPI_MEMBERID || !aPI_MEMBERID.contains("&") || aPI_MEMBERID.split("&").length != 2) {
            log.error("[AA支付]-[请求支付]-“支付通道商号”输入数据格式为【中间使用&分隔】：商户号&通道类型编码值（向第三方获取当前使用通道编码值）" );
            throw new PayException("[AA支付]-[请求支付]-“支付通道商号”输入数据格式为【中间使用&分隔】：商户号&通道类型编码值（向第三方获取当前使用通道编码值）" );
        }
        Map<String, String> payParam = new TreeMap<String, String>() {
            {
                put(aa_merchant, channelWrapper.getAPI_MEMBERID().split("&")[0]);
                put(aa_amount,  channelWrapper.getAPI_AMOUNT());
//                put(aa_pay_type,channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
                put(aa_pay_type,channelWrapper.getAPI_MEMBERID().split("&")[1]);
