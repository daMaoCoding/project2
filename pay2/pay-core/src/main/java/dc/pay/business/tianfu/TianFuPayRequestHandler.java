package dc.pay.business.tianfu;
 ...
 * Apr 30, 2018
 */
@RequestPayHandler("TIANFU")
public final class TianFuPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(TianFuPayRequestHandler.class);
	//wap跳转属性
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        ...
        log.debug("[天付]-[请求支付]-1.组装请求参数完成：" + JSON.toJSONString(payParam));
        return payParam;
    }
    ...
    protected String buildPaySign(Map<String,String> api_response_params) throws PayException {
        ...
        log.debug("[天付]-[请求支付]-2.生成加密URL签名完成：" + JSON.toJSONString(signMd5));
        return signMd5;
    }

    protected List<Map<String, String>> sendRequestGetResult(Map<String, String> payParam, String pay_md5sign) throws PayException {
    	payParam.put(channelWrapper.getAPI_CHANNEL_SIGN_PARAM_NAME(), pay_md5sign);
    	String tmpStr = RestTemplateUtil.postForm(channelWrapper.getAPI_CHANNEL_BANK_URL(), payParam,"UTF-8").trim();
    	if (null == tmpStr || StringUtils.isBlank(tmpStr)) {
    		log.error("[天付]-[请求支付]-3.1.发送支付请求，获取支付请求返回值异常:返回空");
    	...
    	if (null == jsonObject || (jsonObject.containsKey("respcd")  && !"0000".equals(jsonObject.getString("respcd")))) {
			log.error("[天付]-[请求支付]-3.2..发送支付请求，及获取支付请求结果：" + UnicodeUtil.unicodeToString(tmpStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
    		throw new PayException(UnicodeUtil.unicodeToString(tmpStr));
    	}
        String dataStr = jsonObject.getString("data");
        if (StringUtils.isBlank(dataStr)) {
        	log.error("[天付]-[请求支付]-3.3.发送支付请求，获取支付请求返回值异常:data返回空");
        ...
        log.debug("[天付]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：" + JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[天付]-[请求支付]-4.处理请求响应成功：" + JSON.toJSONString(requestPayResult));
