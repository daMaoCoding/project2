 ...
 * 04 20, 2019
 */
@RequestPayHandler("HUANLEZHIFU")
public final class HuanLeZhiFuPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(HuanLeZhiFuPayRequestHandler.class);

//    参数名称				参数含义				是否必填			参数说明
//    pay_memberid			商户ID				是				商户ID注册后获得
//    pay_orderid			订单号				是				请保证订单号不重复
//    pay_amount			金额					是				订单金额，单位：元，精确到分
//    pay_applydate			订单提交时间			是				订单提交的时间:  如： 2019-1-11 18:52:46
//    pay_bankcode			银行编号				是				银行编码
//    pay_notifyurl			服务端返回地址			是				服务端返回地址. （POST返回数据）
//    pay_callbackurl		页面返回地址			是				页面跳转返回地址  （POST返回数据）
//    pay_productname		商品名称				是	
//    pay_md5sign			MD5签名字段			是				请看MD5签名字段格式
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        ...
        log.debug("[欢乐付支付]-[请求支付]-1.组装请求参数完成：{}" ,JSON.toJSONString(payParam));
        return payParam;
    }
...
//            if (!pay_productname.equals(paramKeys.get(i)) && !pay_attach.equals(paramKeys.get(i)) && StringUtils.isNotBlank(api_response_params.get(paramKeys.get(i)))) {
        ...
        log.debug("[欢乐付支付]-[请求支付]-2.生成加密URL签名完成：{}" ,JSON.toJSONString(signMD5));
        return signMD5;
    }
    ...
    protected List<Map<String, String>> sendRequestGetResult(Map<String, String> payParam, String pay_md5sign) throws PayException {
        ...
        log.debug("[欢乐付支付]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：{}",JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[欢乐付支付]-[请求支付]-4.处理请求响应成功：{}",JSON.toJSONString(requestPayResult));
