package dc.pay.business.baiyifu0;
 ...
 * Jun 8, 2019
 */
@RequestPayHandler("BAIYIFU0")
public final class BaiYiFu0PayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(BaiYiFu0PayRequestHandler.class);

    //参数名称    含义  参数长度    是否必填    备注  样例
    //merId   商户ID    String(19)  是   签约合作方的唯一用户号 1537511014022334504
    private static final String merId                ="merId";
    //appId   应用ID    String(19)  是   商户网站使用的编码格式，统一使用UTF-8   1537511014022334572
    private static final String appId                ="appId";
    //orderNo 商户网站唯一订单号   String(20)  是   商户网站唯一订单号（确保在商户系统中唯一）。  153353868461944
    private static final String orderNo                ="orderNo";
    //totalFee    交易金额    Int(11) 是   金额单位（元） 10-5000 限额
    private static final String totalFee                ="totalFee";
    //channeltype 通道类型    Int(1)  是   通道编码    1支付宝2 微信3网银
    private static final String channeltype                ="channeltype";
    //returnUrl   异步    通知  String（256） 是       http://www.baidu.com
    private static final String returnUrl                ="returnUrl";
    //sign    签名  String(32)  是   参见“签名机制”    32位 小写
...
//    private static final String signature  ="pay_md5sign";
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        ...
        if (null == aPI_MEMBERID || !aPI_MEMBERID.contains("&") || aPI_MEMBERID.split("&").length != 2) {
            log.error("[佰亿付]-[请求支付]-“支付通道商号”输入数据格式为【中间使用&分隔】：商户号merId&应用appId" );
            throw new PayException("[佰亿付]-[请求支付]-“支付通道商号”输入数据格式为【中间使用&分隔】：商户号merId&应用appId" );
        }
        ...
        log.debug("[佰亿付]-[请求支付]-1.组装请求参数完成：{}" ,JSON.toJSONString(payParam));
        return payParam;
    }
     ...
     protected String buildPaySign(Map<String,String> api_response_params) throws PayException {
        ...
        log.debug("[佰亿付]-[请求支付]-2.生成加密URL签名完成：{}" ,JSON.toJSONString(signMd5));
        return signMd5;
    }
...
//                log.error("[佰亿付]-[请求支付]-3.1.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
//                throw new PayException(resultStr);
//            }
//            if (!resultStr.contains("{") || !resultStr.contains("}")) {
//                log.error("[佰亿付]-[请求支付]-3.2.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
//                throw new PayException(resultStr);
//            }
            ...
            try {
                ...
                log.error("[佰亿付]-[请求支付]-3.1.发送支付请求，及获取支付请求出错，订单号：" + channelWrapper.getAPI_ORDER_ID() + ",通道名称：" + channelWrapper.getAPI_CHANNEL_BANK_NAME() + ",postUrl:" + channelWrapper.getAPI_CHANNEL_BANK_URL() + ",payForManagerCGIResultJsonObj" + e.getMessage(), e);
...
//                            log.error("[佰亿付]-[请求支付]-3.4.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
//                            throw new PayException(resultStr);
//                        }
//                        result.put(QRCONTEXT, qr);
//                    }
//                }else {
//                    log.error("[佰亿付]-[请求支付]-3.5.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
//                    throw new PayException(resultStr);
//                }
            }else {
                log.error("[佰亿付]-[请求支付]-3.2.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
        ...
        log.debug("[佰亿付]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：{}",JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[佰亿付]-[请求支付]-4.处理请求响应成功：{}",JSON.toJSONString(requestPayResult));
