package dc.pay.business.chengmengzhifu;
 ...
 * @date 15 Aug 2019
 */
@RequestPayHandler("CHENGMENGZHIFU")
public final class ChengMengZhiFuPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(ChengMengZhiFuPayRequestHandler.class);

//    参数名					参数				可空				说明
//    商户ID					arter			N				商户id，由新鼎信分配
//    银行类型				type			N				银行类型，具体请参考附录1
//    金额					value			N				单位元（人民币），2位小数，最小支付金额为0.02
//    商户订单号				orderid			N				商户系统订单号，该订单号将作为新鼎信接口的返回数据。该值需在商户系统内唯一，新鼎信系统暂时不检查该值是否唯一
//    下行异步通知地址			callbackurl		N				下行异步通知过程的返回地址，需要以http://开头且没有任何参数
//    下行同步通知地址			hrefbackurl		Y				下行同步通知过程的返回地址(在支付完成后新鼎信接口将会跳转到的商户系统连接地址)。注：若提交值无该参数，或者该参数值为空，则在支付完成后，新鼎信接口将不会跳转到商户系统，用户将停留在新鼎信接口系统提示支付成功的页面。
//    MD5签名				sign			N				对parter={}&type={}&value={}&orderid={}&callbackurl={}key进行MD5加密，key是商户密钥
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        ...
        log.debug("[诚盟支付]-[请求支付]-1.组装请求参数完成：{}" ,JSON.toJSONString(payParam));
        return payParam;
    }
     ...
     protected String buildPaySign(Map<String,String> api_response_params) throws PayException {
        ...
        log.debug("[诚盟支付]-[请求支付]-2.生成加密URL签名完成：{}" ,JSON.toJSONString(signMD5));
        return signMD5;
    }
    ...
    protected List<Map<String, String>> sendRequestGetResult(Map<String, String> payParam, String pay_md5sign) throws PayException {
        ...
        log.debug("[诚盟支付]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：{}",JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[诚盟支付]-[请求支付]-4.处理请求响应成功：{}",JSON.toJSONString(requestPayResult));
