 ...
 * Dec 18, 2018
 */
@RequestPayHandler("YUNQIZHIFU")
public final class YunQiZhiFuPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(YunQiZhiFuPayRequestHandler.class);
...
//    signature							验签字段					是									MD5加密
  ...
  protected Map<String, String> buildPayParam() throws PayException {
      Map<String, String> payParam = new TreeMap<String, String>() {
          {
              put(branchId, channelWrapper.getAPI_MEMBERID());
              put(orderId,channelWrapper.getAPI_ORDER_ID());
              put(transAmt,channelWrapper.getAPI_AMOUNT());
              put(returnUrl,channelWrapper.getAPI_CHANNEL_BANK_NOTIFYURL());
              put(bizType,channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG().split(",")[0]);
      ...
      log.debug("[云起支付]-[请求支付]-1.组装请求参数完成：{}" ,JSON.toJSONString(payParam));
      return payParam;
  }
...
//          if (!pay_productname.equals(paramKeys.get(i)) && !pay_attach.equals(paramKeys.get(i)) && StringUtils.isNotBlank(api_response_params.get(paramKeys.get(i)))) {
      ...
      log.debug("[云起支付]-[请求支付]-2.生成加密URL签名完成：{}" ,JSON.toJSONString(signMD5));
      return signMD5;
  }
  ...
  protected List<Map<String, String>> sendRequestGetResult(Map<String, String> payParam, String pay_md5sign) throws PayException {
      ...
      postParam.put("branchId", channelWrapper.getAPI_MEMBERID());
      ...
      log.debug("[云起支付]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：{}",JSON.toJSONString(payResultList));
  ...
  protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
      ...
      log.debug("[云起支付]-[请求支付]-4.处理请求响应成功：{}",JSON.toJSONString(requestPayResult));
