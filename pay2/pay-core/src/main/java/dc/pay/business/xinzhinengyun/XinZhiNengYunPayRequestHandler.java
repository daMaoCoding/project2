 ...
 * May 20, 2019
 */
@RequestPayHandler("XINZHINENGYUN")
public final class XinZhiNengYunPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(XinZhiNengYunPayRequestHandler.class);

    //#   参数名 含义  类型  说明
    //1   uid 商户uid   string(24)  必填。您的商户号，在商户后台可以看到。
    private static final String uid                ="uid";
    //2   price   价格  float   必填。单位：元。精确小数点后2位
    private static final String price                ="price";
    //3   istype  支付渠道    int 必填。10001：支付宝扫码；10003：支付宝转账；20001：微信扫码
    private static final String istype                ="istype";
    //4   notify_url  通知回调网址  string(255) 如果不填，以商户后台配置的异步通知地址为准。用户支付成功后，我们服务器会主动发送一个post消息到这个网址。由您自定义。不要urlencode。例：http://www.aaa.com/aacallback
    private static final String notify_url                ="notify_url";
    //5   orderid 商户自定义订单号    string(50)  必填。我们会据此判别是同一笔订单还是新订单。我们回调时，会带上这个参数。例：1533774868
    private static final String orderid                ="orderid";
    //6   orderuid    商户自定义客户号    string(100) 选填。我们会显示在您后台的订单列表中，方便您看到是哪个用户的付款，方便后台对账。强烈建议填写。可以填用户名，也可以填您数据库中的用户uid。
    private static final String orderuid                ="orderuid";
    //7   goodsname   商品名称    string(100) 选填。您的商品名称，用来显示在后台的订单名称。如未设置，我们会使用后台商品管理中对应的商品名称
    private static final String goodsname                ="goodsname";
    //8   return_url  跳转地址    string(255) 必填。用户支付成功后，我们会让用户浏览器自动跳转到这个网址。由您自定义。不要urlencode。例：http://www.aaa.com/aaareturn
...
//    private static final String signature  ="signature";
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        ...
        log.debug("[新智能云]-[请求支付]-1.组装请求参数完成：{}" ,JSON.toJSONString(payParam));
        return payParam;
    }
    ...
    protected String buildPaySign(Map<String,String> api_response_params) throws PayException {
        ...
        log.debug("[新智能云]-[请求支付]-2.生成加密URL签名完成：{}" ,JSON.toJSONString(signMd5));
        return signMd5;
    }
...
//            //    log.error("[新智能云]-[请求支付]-3.1.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
//            //    throw new PayException(resultStr);
//            //    //log.error("[新智能云]-[请求支付]-3.1.发送支付请求，获取支付请求返回值异常:返回空,参数：{}",JSON.toJSONString(map));
...
//        }
        ...
        log.debug("[新智能云]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：{}",JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[新智能云]-[请求支付]-4.处理请求响应成功：{}",JSON.toJSONString(requestPayResult));
