 ...
 * Jul 9, 2019
 */
@RequestPayHandler("JINXIANGZHIFU")
public final class JinXiangZhiFuPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(JinXiangZhiFuPayRequestHandler.class);
    ...
    //way 整数 交易方式 1、微信扫码 2、 支付宝扫码 3、微信 WAP 4、 支付宝      WAP 
    private static final String way                ="way";
    //back 字符串 支付完成的前台跳转地址
    private static final String back                ="back";
    //notify 字符串 后台回调的通知地址
    private static final String notify                ="notify";
    //time 字符串 当前的时间戳，例如 1520656824
    private static final String time                ="time";
    //sign 字符串 签名，详见支付请求的签名方式
...
//            log.error("[金象支付]-[请求支付]-“支付通道商号”输入数据格式为【中间使用&分隔】：商户号&机构号" );
//            throw new PayException("[金象支付]-[请求支付]-“支付通道商号”输入数据格式为【中间使用&分隔】：商户号&机构号" );
//        }
        ...
        log.debug("[金象支付]-[请求支付]-1.组装请求参数完成：{}" ,JSON.toJSONString(payParam));
        return payParam;
    }
    ...
    protected String buildPaySign(Map<String,String> api_response_params) throws PayException {
        ...
        signSrc.append(api_response_params.get(oid));
        signSrc.append(api_response_params.get(amt));
        signSrc.append(api_response_params.get(way));
        signSrc.append(api_response_params.get(back));
        signSrc.append(api_response_params.get(notify));
        signSrc.append(channelWrapper.getAPI_KEY());
        //删除最后一个字符
        //signSrc.deleteCharAt(paramsStr.length()-1);
        String paramsStr = signSrc.toString();
        String signMd5 = HandlerUtil.getMD5UpperCase(paramsStr).toLowerCase();
        log.debug("[金象支付]-[请求支付]-2.生成加密URL签名完成：{}" ,JSON.toJSONString(signMd5));
        return signMd5;
    }
...
//            //    log.error("[金象支付]-[请求支付]-3.1.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
//            //    throw new PayException(resultStr);
//            //    //log.error("[金象支付]-[请求支付]-3.1.发送支付请求，获取支付请求返回值异常:返回空,参数：{}",JSON.toJSONString(map));
...
//        }
        ...
        log.debug("[金象支付]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：{}",JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[金象支付]-[请求支付]-4.处理请求响应成功：{}",JSON.toJSONString(requestPayResult));
