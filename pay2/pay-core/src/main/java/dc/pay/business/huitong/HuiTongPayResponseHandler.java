 ...
 * Jun 13, 2018
 */
@ResponsePayHandler("HUITONG")
public final class HuiTongPayResponseHandler extends PayResponseHandler {
	...
	//回传参数				return_params		String(128)			否			如果商户支付请求时传递了该参数，则通知商户支付成功时会回传该参数
	//支付平台订单号			trade_no			String				是			支付平台里唯一的订单号
	//支付平台订单时间		trade_time			Date				是			支付平台订单交易时间，格式为yyyy-MM-dd HH:mm:ss例如：2015-01-01 12:45:52
	//交易状态				trade_status		String				是			success 交易成功	failed 交易失败	paying 交易中
	//通知类型				notify_type			String				是			back_notify 异步通知	bank_page 页面通知
...
//	private static final String notify_type					="notify_type";
    ...
    public String processForOrderId(Map<String, String> API_RESPONSE_PARAMS) throws PayException {
        ...
        log.debug("[汇通]-[响应支付]-1.获取支付通道响应信息中的订单号完成：" + ordernumberR);
        return ordernumberR;
    }
    ...
    protected String buildPaySign(Map<String, String> api_response_params, String api_key) throws PayException {
        ...
        log.debug("[汇通]-[响应支付]-2.生成加密URL签名完成：" + JSON.toJSONString(signMd5) );
        return signMd5;
    }
    ...
    protected boolean checkPayStatusAndMount(Map<String, String> api_response_params, String db_amount) throws PayException {
        ...
        if (checkAmount && payStatusCode.equalsIgnoreCase("success")) {
            ...
            log.error("[汇通]-[响应支付]金额及状态验证错误,订单号：" + channelWrapper.getAPI_ORDER_ID() + ",第三方支付状态：" + payStatusCode + " ,支付金额：" + responseAmount + " ，应支付金额：" + db_amount);
        }
        log.debug("[汇通]-[响应支付]-3.验证第三方支付响应支付状态&验证第三方支付金额与数据库订单支付金额完成,验证结果：" + my_result + " ,金额验证：" + checkAmount + " ,responseAmount=" + responseAmount + " ,数据库金额：" + db_amount + ",第三方响应支付成功标志:" + payStatusCode + " ,计划成功：success");
        return my_result;
    }
    ...
    protected boolean checkSignMd5(Map<String, String> api_response_params, String signMd5) {
        ...
        log.debug("[汇通]-[响应支付]-4.验证MD5签名：" + my_result);
        return my_result;
    }

    @Override
    protected String responseSuccess() {
        log.debug("[汇通]-[响应支付]-5.第三方支付确认收到消息返回内容：" + RESPONSE_PAY_MSG);
