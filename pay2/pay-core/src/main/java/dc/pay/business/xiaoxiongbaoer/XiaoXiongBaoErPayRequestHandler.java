package dc.pay.business.xiaoxiongbaoer;
 ...
 * Nov 19, 2018
 */
@RequestPayHandler("XIAOXIONGBAOER")
public final class XiaoXiongBaoErPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(XiaoXiongBaoErPayRequestHandler.class);
...
//                put(money,  HandlerUtil.getYuan(channelWrapper.getAPI_AMOUNT()));
        ...
        log.debug("[小熊宝2]-[请求支付]-1.组装请求参数完成：{}" ,JSON.toJSONString(payParam));
        return payParam;
    }
    ...
    protected String buildPaySign(Map<String,String> api_response_params) throws PayException {
        ...
        log.debug("[小熊宝2]-[请求支付]-2.生成加密URL签名完成：{}" ,JSON.toJSONString(signMd5));
        return signMd5;
    }
...
//        else {
            ...
            if (StringUtils.isBlank(resultStr)) {
                log.error("[小熊宝2]-[请求支付]-3.1.发送支付请求，获取支付请求返回值异常:返回空,参数：{}",JSON.toJSONString(payParam));
                throw new PayException("返回空,参数："+JSON.toJSONString(payParam));
            }
//            if (handlerUtil.isWapOrApp(channelWrapper)) {
//                if (channelWrapper.getAPI_CHANNEL_BANK_NAME().contains("XIAOXIONGBAO_BANK_WAP_ZFB_SM")  || channelWrapper.getAPI_CHANNEL_BANK_NAME().contains("XIAOXIONGBAO_BANK_WAP_JD_SM")  ) {
//                    String src = Jsoup.parse(resultStr).select("[id=show_qrcode]").first().attr("src");
//                    if (StringUtils.isBlank(src)) {
//                        log.error("[小熊宝2]-[请求支付]-3.2.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
//                        throw new PayException(resultStr);
//                    }
//                    String byBase64 = QRCodeUtil.decodeByBase64(src);
//                    if (StringUtils.isBlank(byBase64)) {
//                        log.error("[小熊宝2]-[请求支付]-3.3.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
//                        throw new PayException(resultStr);
//                    }
//
//                    //注：第三方告知，不支持京东wap,和微信wap,文档写的是 京东wap扫码，是扫码,不是wap;
//                    if(channelWrapper.getAPI_CHANNEL_BANK_NAME().equalsIgnoreCase("XIAOXIONGBAO_BANK_WAP_JD_SM") || channelWrapper.getAPI_CHANNEL_BANK_NAME().equalsIgnoreCase("XIAOXIONGBAO_BANK_WAP_WX_SM")  ){
//                        result.put(QRCONTEXT, byBase64);
//                    }else{
...
//                }else {
//                    log.error("[小熊宝2]-[请求支付]-3.1.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
//                    throw new PayException(resultStr);
//                }
...
//        }
        ...
        log.debug("[小熊宝2]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：{}",JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[小熊宝2]-[请求支付]-4.处理请求响应成功：{}",JSON.toJSONString(requestPayResult));
