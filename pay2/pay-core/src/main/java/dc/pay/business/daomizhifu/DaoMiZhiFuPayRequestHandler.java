...
@RequestPayHandler("DAOMIZHIFU")
public final class DaoMiZhiFuPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(DaoMiZhiFuPayRequestHandler.class);


    private static final String      app_id = "app_id";//	√	√	参数名称：商家号 商户签约时，平台分配给商家的唯一身份标识。例如：1803110116
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        ...
        log.debug("[稻米支付]-[请求支付]-1.组装请求参数完成：" + JSON.toJSONString(payParam));
        return payParam;
    }
    ...
    protected String buildPaySign(Map<String,String> params) throws PayException {
        String paramsStr = String.format("app_id=%s&notify_url=%s&out_trade_no=%s&total_amount=%s&trade_type=%s%s",
                params.get(app_id),
                params.get(notify_url),
                params.get(out_trade_no),
                params.get(total_amount),
                params.get(trade_type),
...
*/
             ...
             log.error("[稻米支付]3.发送支付请求，及获取支付请求结果出错：", e);
             throw new PayException(e.getMessage(), e);
        }
        log.debug("[稻米支付]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：{}" ,JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[稻米支付]-[请求支付]-4.处理请求响应成功：" + JSON.toJSONString(requestPayResult));
