 ...
 * Aug 27, 2019
 */
@RequestPayHandler("YOUMIZHIFU")
public final class YouMiZhiFuPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(YouMiZhiFuPayRequestHandler.class);

    //参数(POST)    必填/选填   说明  示例
    //merchant_id 必填  商户ID、在平台首页右边获取商户ID  100030
    private static final String merchant_id                ="merchant_id";
    //content_type    必填  返回数据类型  json
    private static final String content_type                ="content_type";
    //pay_type    必填  支付方式    1:支付宝扫码    2:微信扫码    3:支付宝H5    4:微信H5    100 - 支付宝（所有支付宝都可以用H5和扫码）    101 - 微信（所有微信都可以用H5和扫码）
    private static final String pay_type                ="pay_type";
    //out_trade_no    必填  商户订单号，需保证在商户平台唯一（YYYYMMDDHHMMSS+5位数字）   
    private static final String out_trade_no                ="out_trade_no";
    //amount  必填  支付金额(单位：分）  100
    private static final String amount                ="amount";
    //notic_url   选填  同步地址，如果后台开账户是填了固定地址，这里可以不送  
    private static final String notic_url                ="notic_url";
    //return_url  选填  支付成功后网页自动跳转地址，仅在网页类型为form下有效    
...
//    private static final String signature  ="sign";
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        ...
        if (null == aPI_MEMBERID || !aPI_MEMBERID.contains("&") || aPI_MEMBERID.split("&").length != 2) {
            log.error("[有米支付]-[请求支付]-“支付通道商号”输入数据格式为【中间使用&分隔】：商户号&通道类型编码值（向第三方获取当前使用通道编码值）" );
            throw new PayException("[有米支付]-[请求支付]-“支付通道商号”输入数据格式为【中间使用&分隔】：商户号&通道类型编码值（向第三方获取当前使用通道编码值）" );
        }
...
//                put(pay_type,channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
                ...
                put(notic_url,channelWrapper.getAPI_CHANNEL_BANK_NOTIFYURL());
                put(return_url,channelWrapper.getAPI_WEB_URL());
            }
        };
        log.debug("[有米支付]-[请求支付]-1.组装请求参数完成：{}" ,JSON.toJSONString(payParam));
        return payParam;
    }
    ...
    protected String buildPaySign(Map<String,String> api_response_params) throws PayException {
        ...
        log.debug("[有米支付]-[请求支付]-2.生成加密URL签名完成：{}" ,JSON.toJSONString(signMd5));
        return signMd5;
    }
...
//            String resultStr = RestTemplateUtil.postForm(channelWrapper.getAPI_CHANNEL_BANK_URL(), payParam,"UTF-8");
            ...
            //    log.error("[有米支付]-[请求支付]-3.1.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
            //    throw new PayException(resultStr);
            //    //log.error("[有米支付]-[请求支付]-3.1.发送支付请求，获取支付请求返回值异常:返回空,参数：{}",JSON.toJSONString(map));
...
//            System.out.println("请求返回=========>"+resultStr);
        ...
        log.debug("[有米支付]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：{}",JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[有米支付]-[请求支付]-4.处理请求响应成功：{}",JSON.toJSONString(requestPayResult));
