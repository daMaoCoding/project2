 ...
 * Jul 20, 2018
 */
@ResponsePayHandler("HEFUTONG")
public final class HeFuTongPayResponseHandler extends PayResponseHandler {
    ...
    //service                 接口类型 
    //way                     支付方式         
    //time_end                交易结束时间                    支付完成时间，格式为yyyyMMddHHmmss，如2009年02月27日9点00分00秒表示为20090227090000。时区为GMT+8 beijing。该时间取自平台服务器
    //device_info             设备号                          用户传的原样返回给用户
    //attach                  附加信息                        用户传的原样返回给用户
    //result_code             业务结果                        表示成功，非0表示失败
    //sign                    签名加密                        d5(mch_id + time_end + out_trade_no + ordernumber + transtypeid + transaction_id + total_fee + service + way + result_code + 密钥) md5加密的编码为utf-8签名后的字符串不区分大小写
...
//    private static final String attach                          ="attach";
    ...
    public String processForOrderId(Map<String, String> API_RESPONSE_PARAMS) throws PayException {
        ...
        log.debug("[合付通]-[响应支付]-0.获取支付通道响应信息中的订单号完成：{}" ,ordernumberR);
        return ordernumberR;
    }
    ...
    protected String buildPaySign(Map<String, String> api_response_params, String api_key) throws PayException {
        ...
        log.debug("[合付通]-[响应支付]-2.生成加密URL签名完成：{}", JSON.toJSONString(signMd5) );
        return signMd5;
    }
    ...
    protected boolean checkPayStatusAndMount(Map<String, String> api_response_params, String db_amount) throws PayException {
        ...
        if (checkAmount && payStatusCode.equalsIgnoreCase("0")) {
            ...
            log.error("[合付通]-[响应支付]金额及状态验证错误,订单号：" + channelWrapper.getAPI_ORDER_ID() + ",第三方支付状态：" + payStatusCode + " ,支付金额：" + responseAmount + " ，应支付金额：" + db_amount);
        }
        log.debug("[合付通]-[响应支付]-3.验证第三方支付响应支付状态&验证第三方支付金额与数据库订单支付金额完成,验证结果：" + my_result + " ,金额验证：" + checkAmount + " ,responseAmount=" + responseAmount + " ,数据库金额：" + db_amount + ",第三方响应支付成功标志:" + payStatusCode + " ,计划成功：0");
        return my_result;
    }
    ...
    protected boolean checkSignMd5(Map<String, String> api_response_params, String signMd5) {
        ...
        log.debug("[合付通]-[响应支付]-4.验证MD5签名：{}", my_result);
        return my_result;
    }

    @Override
    protected String responseSuccess() {
        log.debug("[合付通]-[响应支付]-5.第三方支付确认收到消息返回内容：{}", RESPONSE_PAY_MSG);
