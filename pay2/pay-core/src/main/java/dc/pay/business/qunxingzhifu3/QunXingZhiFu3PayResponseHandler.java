 ...
 * Aug 21, 2019
 */
@ResponsePayHandler("QUNXINGZHIFU3")
public final class QunXingZhiFu3PayResponseHandler extends PayResponseHandler {
    ...
    //4   price   订单定价    string(50)  一定存在。是您在发起付款接口传入的订单价格
    private static final String price                ="price";
    //5   realprice   实际支付金额  string(50)  一定存在。表示用户实际支付的金额。一般会和price值一致，如果同时存在多个用户支付同一金额，就会和price存在一定差额，差额一般在1-2分钱上下，越多人同时付款，差额越大
...
//    private static final String key        ="key";
    ...
    public String processForOrderId(Map<String, String> API_RESPONSE_PARAMS) throws PayException {
       ...
       //    log.error("[群星支付3]-[响应支付]-“密钥（私钥）框”输入数据格式为【中间使用-分隔】：MD5Key-RSA私钥" );
       //    throw new PayException("[群星支付3]-[响应支付]-“密钥（私钥）框”输入数据格式为【中间使用-分隔】：MD5Key-RSA私钥" );
       //}
...
//        String partnerR = API_RESPONSE_PARAMS.get(merchno);
        ...
        log.debug("[群星支付3]-[响应支付]-1.获取支付通道响应信息中的订单号完成：{}" ,ordernumberR);
        return ordernumberR;
    }
    ...
    protected String buildPaySign(Map<String, String> api_response_params, String api_key) throws PayException {
        ...
        signStr.append(api_response_params.get(user_order_no));
        signStr.append(api_response_params.get(orderno));
        signStr.append(api_response_params.get(tradeno));
        ...
        log.debug("[群星支付3]-[响应支付]-2.生成加密URL签名完成：{}", JSON.toJSONString(signMd5) );
        return signMd5;
    }
...
//        String payStatusCode = api_response_params.get(status);
        ...
        if (checkAmount && payStatusCode.equalsIgnoreCase("无")) {
            ...
            log.error("[群星支付3]-[响应支付]金额及状态验证错误,订单号：" + channelWrapper.getAPI_ORDER_ID() + ",第三方支付状态：" + payStatusCode + " ,支付金额：" + responseAmount + " ，应支付金额：" + db_amount);
        }
        log.debug("[群星支付3]-[响应支付]-3.验证第三方支付响应支付状态&验证第三方支付金额与数据库订单支付金额完成,验证结果：" + my_result + " ,金额验证：" + checkAmount + " ,responseAmount=" + responseAmount + " ,数据库金额：" + db_amount + ",第三方响应支付成功标志:" + payStatusCode + " ,计划成功：无");
        return my_result;
    }
    ...
    protected boolean checkSignMd5(Map<String, String> api_response_params, String signMd5) {
        ...
        log.debug("[群星支付3]-[响应支付]-4.验证MD5签名：{}", my_result);
        return my_result;
    }

    @Override
    protected String responseSuccess() {
        log.debug("[群星支付3]-[响应支付]-5.第三方支付确认收到消息返回内容：{}", RESPONSE_PAY_MSG);
