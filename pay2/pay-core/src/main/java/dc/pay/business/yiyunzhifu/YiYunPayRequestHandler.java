package dc.pay.business.yiyunzhifu;
...
@RequestPayHandler("YIYUN")
public final class YiYunPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(YiYunPayRequestHandler.class);
    ...
    private static final  String  version	     = "version";        //是	固定值3.0
     ...
     private static final String  partner	 = "partner";            //是	商户id,由益云分配
     private static final String  banktype	     = "banktype";       //是	银行类型，default为跳转到益云接口进行选择支付
     ...
     private static final String  callBackUrl	 = "/respPayWeb/YIYUN_BANK_NULL_FOR_CALLBACK/";
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        Map<String, String> payParam = new TreeMap<String, String>() {
            {
                put(version, "3.0");
                put(method, "yy.online.interface");
...
//        }
        ...
        log.debug("[益云]-[请求支付]-1.组装请求参数完成：" + JSON.toJSONString(payParam));
        return payParam;
    }
    ...
    protected String buildPaySign(Map api_response_params) throws PayException {
       // version={0}&method={1}&partner={2}&banktype={3}&paymoney={4}&ordernumber={5}&callbackurl={6}key
        ...
        log.debug("[益云]-[请求支付]-2.生成加密URL签名完成：" + JSON.toJSONString(signMd5));
        return signMd5;
    }
    ...
    protected List<Map<String, String>> sendRequestGetResult(Map<String, String> payParam, String pay_md5sign) throws PayException {
        ...
        try {
                ...
                if(resultStr.toLowerCase().contains("</form>") && ( HandlerUtil.isWY(channelWrapper) ||  HandlerUtil.isWapOrApp(channelWrapper)) ){
                    ...
                    if(resultStr.contains("html") &&  !resultStr.startsWith("{") ) {  //微信扫码转到微信反扫
                        ...
                        if (status.equalsIgnoreCase("1") && StringUtils.isBlank(message) && StringUtils.isNotBlank(qrurl) && !qrurl.contains("空")) {
                            ...
                            log.error("[益云]3.发送支付请求，及获取支付请求结果：" +resultStr + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
            ...
            log.error("[益云]3.发送支付请求，及获取支付请求结果出错：", e);
            throw new PayException(e.getMessage(), e);
        }
        log.debug("[益云]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：" + JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[益云]-[请求支付]-4.处理请求响应成功：" + JSON.toJSONString(requestPayResult));
