package dc.pay.business.liandong;
...
* Nov 28, 2018
*/
@RequestPayHandler("LIANDONG")
public final class LianDongPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(LianDongPayRequestHandler.class);


    //参数名                  参数               可空         加入签名         说明
    //商户ID                  parter              N              Y             商户id，由联动支付分配
    //接口类型                type                N              Y             类型，具体请参考附录1
    //金额                    value               N              Y             单位元（人民币），2位小数，金额需要为100的倍数,包括100,否则提示错误,最高3000每笔
    //商户订单号              orderid             N              Y             商户系统订单号，该订单号将作为联动支付接口的返回数据。该值需在商户系统内唯一，联动支付系统暂时不检查该值是否唯一
    //下行异步通知地址        callbackurl         N              Y             下行异步通知过程的返回地址，需要以http://开头且没有任何参数
    //下行同步通知地址        hrefbackurl         Y              N             下行同步通知过程的返回地址(在支付完成后联动支付接口将会跳转到的商户系统连接地址)。注：若提交值无该参数，或者该参数值为空，则在支付完成后，联动支付接口将不会跳转到商户系统，用户将停留在联动支付接口系统提示支付成
    //支付用户IP              payerIp             N              N             用户在下单时的真实IP，联动支付接口将会判断玩家支付时的ip和该值是否相同。若不相同，联动支付接口将提示用户支付风险
...
//    private static final String signature  ="sign";
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        ...
        log.debug("[联动]-[请求支付]-1.组装请求参数完成：" + JSON.toJSONString(payParam));
        return payParam;
    }
    ...
    protected String buildPaySign(Map<String,String> api_response_params) throws PayException {
        ...
        log.debug("[联动]-[请求支付]-2.生成加密URL签名完成：{}" ,JSON.toJSONString(signMd5));
        return signMd5;
    }
...
//                log.error("[联动]-[请求支付]-3.1.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
//                throw new PayException(resultStr);
//            }
////            resultStr = UnicodeUtil.unicodeToString(resultStr);
//            if (resultStr.contains("errotMsg")) {
//                log.error("[联动]-[请求支付]-3.3.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
//                throw new PayException(resultStr);
//            }
...
////            } catch (Exception e) {
////                e.printStackTrace();
////                log.error("[联动]-[请求支付]-3.4.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
////                throw new PayException(resultStr);
////            }
////            //只取正确的值，其他情况抛出异常
...
////                }
////            }else {
////                log.error("[联动]-[请求支付]-3.5.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
////                    throw new PayException(resultStr);
////            }
...
//        }
        ...
        log.debug("[联动]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：{}",JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[联动]-[请求支付]-4.处理请求响应成功：{}",JSON.toJSONString(requestPayResult));
