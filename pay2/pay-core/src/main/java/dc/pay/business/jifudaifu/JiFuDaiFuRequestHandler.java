...
@RequestDaifuHandler("JIFUDAIFU")
public final class JiFuDaiFuRequestHandler extends DaifuRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(JiFuDaiFuRequestHandler.class);
    ...
    protected PayEumeration.DAIFU_RESULT requestDaifuAllInOne(Map<String,String> payParam,Map<String,String> details) throws PayException {
        ...
        try {
              ...
              String cipherRes = JiFuPayUtils.encrypt(contentJSON,channelWrapper.getAPI_PUBLIC_KEY());
               ...
               //生成签名
                try{
                    signRes = JiFuPayUtils.sign(payParam,channelWrapper.getAPI_KEY());
                }catch (Exception e){
                   log.error(e.getMessage(),e);
                    details.put(RESPONSEKEY, "[即付代付]生成签名错误，请检查公钥私钥。");
   ...
   // 其他情况抛异常
    ...
    protected PayEumeration.DAIFU_RESULT queryDaifuAllInOne(Map<String, String> payParam,Map<String, String> details) throws PayException {
       if(1==2) throw new PayException("[即付代付][代付][查询订单状态]该功能未完成。");
        try {
            //组装参数
            payParam.put(version,"v1");
            payParam.put(merchantId,channelWrapper.getAPI_MEMBERID());
            ...
            //生成签名
            String signRes = JiFuPayUtils.sign(payParam,channelWrapper.getAPI_KEY());
            payParam.put(channelWrapper.getAPI_CHANNEL_SIGN_PARAM_NAME(),signRes);
    ...
    protected long queryDaifuBalanceAllInOne(Map<String,String> payParam,Map<String,String> details) throws PayException {
        if(1==2) throw new PayException("[即付代付][代付余额查询]该功能未完成。");
        ...
        try {
            ...
            if(HandlerUtil.valJsonObj(jsonObj,code,"00000") && StringUtils.isNotBlank(jsonObj.getString(cipher)) ){
                String bizContent="";
                if(StringUtils.isNotBlank(jsonObj.getString(cipher))) bizContent = JiFuPayUtils.decrypt(jsonObj.getString(cipher),channelWrapper.getAPI_KEY());
            ...
            log.error("[即付代付][代付余额查询]出错,错误消息：{},参数：{}",e.getMessage(),JSON.toJSONString(payParam),e);
            throw new PayException( String.format("[即付代付][代付余额查询]出错,错误:%s",e.getMessage()));
        }
    ...
    private PayEumeration.DAIFU_RESULT getDaifuResult(String resultStr,boolean isQuery) throws PayException {
        ...
        //验证第三方返回签名
        try {
            b=JiFuPayUtils.verify(responseParams,signResp,channelWrapper.getAPI_PUBLIC_KEY());
         ...
         if(b){
                if(responseParams.get(code).equals("00000")) {
                    ...
                    try {
                        if(StringUtils.isNotBlank(responseParams.get(cipher))) bizContent = JiFuPayUtils.decrypt(responseParams.get(cipher),channelWrapper.getAPI_KEY());
