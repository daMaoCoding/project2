package dc.pay.business.dashuzhifu;
 ...
 * Aug 15, 2019
 */
@RequestPayHandler("DASHUZHIFU")
public final class DaShuZhiFuPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(DaShuZhiFuPayRequestHandler.class);

    //参数名 参数  必填  可空  加入签名    说明
    //商户ID    parter  Y   N   Y   商户id，由大树支付分配
    ...
    //商户订单号   orderid Y   N   Y   商户系统订单号，该订单号将作为大树支付接口的返回数据。该值需在商户系统内唯一，大树支付系统暂时不检查该值是否唯一
...
//    private static final String signature  ="sign";
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        ...
        if (null == aPI_MEMBERID || !aPI_MEMBERID.contains("&") || aPI_MEMBERID.split("&").length != 2) {
            log.error("[大树支付]-[请求支付]-“支付通道商号”输入数据格式为【中间使用&分隔】：商户号&通道类型编码值（向第三方获取当前使用通道编码值）" );
            throw new PayException("[大树支付]-[请求支付]-“支付通道商号”输入数据格式为【中间使用&分隔】：商户号&通道类型编码值（向第三方获取当前使用通道编码值）" );
        }
        Map<String, String> payParam = new TreeMap<String, String>() {
            {
                put(parter, channelWrapper.getAPI_MEMBERID().split("&")[0]);
//                put(b_type,channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
                put(b_type,channelWrapper.getAPI_MEMBERID().split("&")[1]);
                ...
                put(goodsinfo,"name");
                put(attach,  channelWrapper.getAPI_MEMBERID().split("&")[0]);
                put(nonce_str,  HandlerUtil.getRandomStr(6));
            }
        };
        log.debug("[大树支付]-[请求支付]-1.组装请求参数完成：{}" ,JSON.toJSONString(payParam));
        return payParam;
    }
    ...
    protected String buildPaySign(Map<String,String> api_response_params) throws PayException {
        ...
        log.debug("[大树支付]-[请求支付]-2.生成加密URL签名完成：{}" ,JSON.toJSONString(signMd5));
        return signMd5;
    }
...
//            result.put(HTMLCONTEXT, HandlerUtil.getHtmlContent(channelWrapper.getAPI_CHANNEL_BANK_URL(),payParam).toString().replace("method='post'","method='get'"));
            result.put(JUMPURL, HandlerUtil.getHtmlUrl(channelWrapper.getAPI_CHANNEL_BANK_URL(), payParam));
        }
...
//            String resultStr = RestTemplateUtil.sendByRestTemplateRedirect(channelWrapper.getAPI_CHANNEL_BANK_URL(), payParam, String.class, HttpMethod.GET);
            ...
            //    log.error("[大树支付]-[请求支付]-3.1.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
            //    throw new PayException(resultStr);
            //    //log.error("[大树支付]-[请求支付]-3.1.发送支付请求，获取支付请求返回值异常:返回空,参数：{}",JSON.toJSONString(map));
        ...
        log.debug("[大树支付]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：{}",JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[大树支付]-[请求支付]-4.处理请求响应成功：{}",JSON.toJSONString(requestPayResult));
