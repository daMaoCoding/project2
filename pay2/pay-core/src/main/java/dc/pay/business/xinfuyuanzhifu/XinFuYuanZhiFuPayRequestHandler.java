package dc.pay.business.xinfuyuanzhifu;
 ...
 * Jul 4, 2019
 */
@RequestPayHandler("XINFUYUANZHIFU")
public final class XinFuYuanZhiFuPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(XinFuYuanZhiFuPayRequestHandler.class);
...
//            log.error("[新富源支付]-[请求支付]-“支付通道商号”输入数据格式为【中间使用&分隔】：商户号&机构号" );
//            throw new PayException("[新富源支付]-[请求支付]-“支付通道商号”输入数据格式为【中间使用&分隔】：商户号&机构号" );
//        }
        ...
        log.debug("[新富源支付]-[请求支付]-1.组装请求参数完成：{}" ,JSON.toJSONString(payParam));
        return payParam;
    }
...
//            if (!bankCode.equals(paramKeys.get(i)) && StringUtils.isNotBlank(api_response_params.get(paramKeys.get(i)))) {
        ...
        log.debug("[新富源支付]-[请求支付]-2.生成加密URL签名完成：{}" ,JSON.toJSONString(signMd5));
        return signMd5;
    }
...
//            String resultStr = RestTemplateUtil.sendByRestTemplateRedirect(channelWrapper.getAPI_CHANNEL_BANK_URL(), payParam, String.class, HttpMethod.POST);
            ...
            //    log.error("[新富源支付]-[请求支付]-3.1.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
            //    throw new PayException(resultStr);
            //    //log.error("[新富源支付]-[请求支付]-3.1.发送支付请求，获取支付请求返回值异常:返回空,参数：{}",JSON.toJSONString(map));
...
//                result.put( handlerUtil.isWapOrApp(channelWrapper) ? JUMPURL : QRCONTEXT, code_url);
                ...
                //小肉肉 2019/8/28 10:47:22
                //哪个通道需要使用你们的返回值在我平台包装成二维码 ？
                //小肉肉 2019/8/28 10:47:27
                //@凌妍 
                //凌妍 2019/8/28 10:47:41
                //银联扫码
                //11:00:46
                //Biubi 2019/8/28 11:00:46
                //扫码的都是要
                if (handlerUtil.isWEBWAPAPP_SM(channelWrapper)) {
                    result.put( QRCONTEXT, code_url);
                }else {
                    result.put( code_url.contains("<body>") ? HTMLCONTEXT : JUMPURL, code_url);
                }
        ...
        log.debug("[新富源支付]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：{}",JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[新富源支付]-[请求支付]-4.处理请求响应成功：{}",JSON.toJSONString(requestPayResult));
