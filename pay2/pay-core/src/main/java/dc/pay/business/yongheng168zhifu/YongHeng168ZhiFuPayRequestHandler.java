package dc.pay.business.yongheng168zhifu;
 ...
 * Jun 28, 2019
 */
@RequestPayHandler("YONGHENG168ZHIFU")
public final class YongHeng168ZhiFuPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(YongHeng168ZhiFuPayRequestHandler.class);
    ...
    //pay_notifyurl 服务端通知   是   是   请务必填写此字段 服务端返回地址.（POST返回数据）
    ...
    //pay_attach    附加字段    否(网关支付必填)   否   此字段在返回时按原样返回 (中文需要url编码)    (2019/4/1) 注：    对接网关时，需要在收银台页面展示商户logo和名字，所以网关支付请在此字段添加如下信息 logo=图片地址&name=名称, 其中图片地址和名称需要 url 编码
...
//    private static final String signature  ="pay_md5sign";
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        ...
        if (null == aPI_MEMBERID || !aPI_MEMBERID.contains("&") || aPI_MEMBERID.split("&").length != 2) {
            log.error("[永恒168支付]-[请求支付]-“支付通道商号”输入数据格式为【中间使用&分隔】：商户号&通道类型编码值（向第三方获取当前使用通道编码值）" );
            throw new PayException("[永恒168支付]-[请求支付]-“支付通道商号”输入数据格式为【中间使用&分隔】：商户号&通道类型编码值（向第三方获取当前使用通道编码值）" );
        }
...
//                put(pay_callbackurl,channelWrapper.getAPI_WEB_URL());
                ...
                //pay_attach    附加字段    否(网关支付必填)   否   此字段在返回时按原样返回 (中文需要url编码)    (2019/4/1) 
                //注：    对接网关时，需要在收银台页面展示商户logo和名字，所以网关支付请在此字段添加如下信息 logo=图片地址&name=名称, 其中图片地址和名称需要 url 编码
                put(pay_attach,handlerUtil.UrlEncode("logo=https://www.baidu.com/&name=name"));
                put(pay_productname,"name");
            }
        };
        log.debug("[永恒168支付]-[请求支付]-1.组装请求参数完成：{}" ,JSON.toJSONString(payParam));
        return payParam;
    }
     ...
     protected String buildPaySign(Map<String,String> api_response_params) throws PayException {
        ...
        for (int i = 0; i < paramKeys.size(); i++) {
            if (!pay_productname.equals(paramKeys.get(i)) && !pay_attach.equals(paramKeys.get(i))) {                
        ...
        log.debug("[永恒168支付]-[请求支付]-2.生成加密URL签名完成：{}" ,JSON.toJSONString(signMd5));
        return signMd5;
    }
...
////                log.error("[永恒168支付]-[请求支付]-3.1.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
////                throw new PayException(resultStr);
////            }
////            if (!resultStr.contains("{") || !resultStr.contains("}")) {
////                log.error("[永恒168支付]-[请求支付]-3.2.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
////                throw new PayException(resultStr);
////            }
...
//            } catch (Exception e) {
//                log.error("[永恒168支付]-[请求支付]-3.1.发送支付请求，及获取支付请求出错，订单号：" + channelWrapper.getAPI_ORDER_ID() + ",通道名称：" + channelWrapper.getAPI_CHANNEL_BANK_NAME() + ",postUrl:" + channelWrapper.getAPI_CHANNEL_BANK_URL() + ",payForManagerCGIResultJsonObj" + e.getMessage(), e);
...
//        }
        ...
        log.debug("[永恒168支付]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：{}",JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[永恒168支付]-[请求支付]-4.处理请求响应成功：{}",JSON.toJSONString(requestPayResult));
