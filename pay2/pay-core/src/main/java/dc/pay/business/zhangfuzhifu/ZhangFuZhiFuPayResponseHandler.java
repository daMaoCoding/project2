package dc.pay.business.zhangfuzhifu;
 ...
 * Jan 28, 2019
 */
@ResponsePayHandler("ZHANGFUZHIFU")
public final class ZhangFuZhiFuPayResponseHandler extends PayResponseHandler {
    private final Logger log = LoggerFactory.getLogger(getClass());
// merchant_code	String(12)	√	参数名称：商家号	商户签约时，掌付分配给商家的唯一身份标识。例如：800003004321
// notify_type	String(14)	√	参数名：通知方式	固定值：offline_notify
// notify_id	String(100)	√	参数名：通知校验ID	此版本不需要校验，但是参数依然保留
// interface_version	String(10)	√	参数名称：接口版本	固定值：V3.0(大写)
// sign_type	String(10)	√	参数名称：签名方式1.取值为：RSA或RSA-S2.该字段不参与签名
// sign	String	√	参数名称：掌付返回签名数据	该参数用于验签，值如何使用，请参考掌付提供的示例代码			业务参数
// order_no	String(64)	√	参数名称：商家订单号	商家网站生成的订单号，由商户保证其唯一性，由字母、数字、下划线组成。
// order_time	Date	√	参数名称：商家订单时间	时间格式：yyyy-MM-dd HH:mm:ss
// order_amount	Number(13,2)	√	参数名称：商家订单金额	以元为单位，精确到小数点后两位.例如：12.01
// extra_return_param	String(100)	×	参数名称：回传参数	商户如果支付请求是传递了该参数，则通知商户支付成功时会回传该参数
// trade_no	String(30)	√	参数名：掌付订单号
// trade_time	Date	√	参数名：掌付订单时间	格式：yyyy-MM-dd HH:mm:ss
// trade_status	String(7)	√	参数名：订单状态	取值为“SUCCESS”，代表订单交易成功
    ...
    public String processForOrderId(Map<String, String> API_RESPONSE_PARAMS) throws PayException {
        ...
        log.debug("[ZF支付]-[响应支付]-1.获取支付通道响应信息中的订单号完成：{}" ,ordernumberR);
        return ordernumberR;
    }
...
//		    String wpay_public_key = "MFwwDQYJKoZIhvcNAQEBBQADSwAwSAJBAKpfMjR7vhZU7HkgQa2hI5nYDzpzMXrwfmZBj4HVE0bL7hgJUkstCcBe86d0Z5iFOvrWj5zAbIO56ixWtTKUxeMCAwEAAQ==";
	    ...
	    log.debug("[ZF支付]-[请求支付]-2.生成加密URL签名完成：" + JSON.toJSONString(result));
...
//        boolean checkAmount =  HandlerUtil.isAllowAmountt(db_amount,responseAmount,"100");//我平台默认允许一元偏差
        ...
        if (checkAmount && payStatusCode.equalsIgnoreCase("SUCCESS")) {
            ...
            log.error("[ZF支付]-[响应支付]金额及状态验证错误,订单号：" + channelWrapper.getAPI_ORDER_ID() + ",第三方支付状态：" + payStatusCode + " ,支付金额：" + responseAmount + " ，应支付金额：" + db_amount);
        }
        log.debug("[ZF支付]-[响应支付]-3.验证第三方支付响应支付状态&验证第三方支付金额与数据库订单支付金额完成,验证结果：" + my_result + " ,金额验证：" + checkAmount + " ,responseAmount=" + responseAmount + " ,数据库金额：" + db_amount + ",第三方响应支付成功标志:" + payStatusCode + " ,计划成功：SUCCESS");
        return my_result;
    }
    ...
    protected boolean checkSignMd5(Map<String, String> api_response_params, String signMd5) {
        ...
        log.debug("[ZF支付]-[响应支付]-4.验证MD5签名：{}", my_result);
        return my_result;
    }
    ...
    protected String responseSuccess() {
        log.debug("[ZF支付]-[响应支付]-5.第三方支付确认收到消息返回内容：{}", RESPONSE_PAY_MSG);
