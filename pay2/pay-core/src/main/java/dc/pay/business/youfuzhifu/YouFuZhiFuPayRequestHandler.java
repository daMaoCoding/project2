...
@RequestPayHandler("YOUFUZHIFU")
public final class YouFuZhiFuPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(YouFuZhiFuPayRequestHandler.class);
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        ...
        log.debug("[优付支付]-[请求支付]-1.组装请求参数完成：" + JSON.toJSONString(payParam));
        return payParam;
    }
    ...
    protected List<Map<String, String>> sendRequestGetResult(Map<String, String> payParam, String pay_md5sign) throws PayException {
        ...
        try {
                ...
                if ( HandlerUtil.isWY(channelWrapper) || HandlerUtil.isYLKJ(channelWrapper)  ||HandlerUtil.isWapOrApp(channelWrapper) ||channelWrapper.getAPI_CHANNEL_BANK_NAME().endsWith("WEBWAPAPP_QQ_SM")) {
                    ...
                    payResultList.add(result);
                       ...
                       //<script>window.location.href='http://api.unpay.com/PayForm.aspx?Data=MzAwNDI5MjAxODA1MjUwMzI1NTQ1MDUwNDg='</script>
                        ...
                        if(StringUtils.isNotBlank(resultStr) && resultStr.contains(location)) {
                            String scriptUrl = Jsoup.parse(resultStr).getElementsByTag("script").first().html();
                            if(StringUtils.isNotBlank(scriptUrl)){
                                scriptUrl = scriptUrl.replaceAll("'","");
                                scriptUrl = scriptUrl.substring(scriptUrl.indexOf(".href=")+6,scriptUrl.length());
                                HtmlPage htmlPage = HttpUtil.sendByHtmlUnit(scriptUrl);
                                final HtmlImage htmlImage = (HtmlImage) htmlPage.getByXPath("//img[@id='Image']").get(0);
                                qrContent = htmlImage.getSrcAttribute();
                                if(StringUtils.isNotBlank(qrContent)&& qrContent.contains("uuid="))
                                    qrContent = qrContent.substring(qrContent.indexOf("uuid=")+5,qrContent.length());
                             }
                            if(StringUtils.isBlank(qrContent)){
                                throw new PayException("无法解析出二维码,第三方首次返回："+resultStr);
                            }
                            result.put(QRCONTEXT, HandlerUtil.UrlDecode(qrContent));
                            payResultList.add(result);
                         }else if(StringUtils.isNotBlank(resultStr) && resultStr.contains("html")){
            ...
            log.error("[优付支付]3.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG(),e);
            throw new PayException("无法解析出二维码,第三方首次返回："+resultStr);
        }
        log.debug("[优付支付]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：{}" ,JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[优付支付]-[请求支付]-4.处理请求响应成功：" + JSON.toJSONString(requestPayResult));
