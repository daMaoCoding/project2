 ...
 * Nov 14, 2018
 */
@ResponsePayHandler("WESUTONG")
public final class WeSuTongPayResponseHandler extends PayResponseHandler {
    ...
    //notify_id                 通知校验ID                String(100)           必选          商家系统接收到此通知消息后，用此校验ID向支付平台校验此通知的合法性，由32位数字和字母组成。例如：e722dceae317466bbf9cc5f1254b8b0a 注：此版本暂不校验，但参数依旧保留。
...
//    private static final String key        ="key";
    ...
    public String processForOrderId(Map<String, String> API_RESPONSE_PARAMS) throws PayException {
        ...
        log.debug("[威速通]-[响应支付]-1.获取支付通道响应信息中的订单号完成：{}" ,ordernumberR);
        return ordernumberR;
    }
    @Override
    protected String buildPaySign(Map<String, String> api_response_params, String api_key) throws PayException {
        ...
        if("RSA-S".equals(api_response_params.get(sign_type))){ // sign_type = "RSA-S"
            result = RsaUtil.validateSignByPublicKey(signInfo, channelWrapper.getAPI_PUBLIC_KEY(), api_response_params.get(signature));    // 验签   signInfo财猫快汇返回的签名参数排序， wpay_public_key财猫快汇公钥， wpaySign财猫快汇返回的签名
        }
        log.debug("[威速通]-[响应支付]-2.生成加密URL签名完成：" + JSON.toJSONString(result) );
    ...
    protected boolean checkPayStatusAndMount(Map<String, String> api_response_params, String db_amount) throws PayException {
        ...
        if (checkAmount && payStatusCode.equalsIgnoreCase("SUCCESS")) {
            ...
            log.error("[威速通]-[响应支付]金额及状态验证错误,订单号：" + channelWrapper.getAPI_ORDER_ID() + ",第三方支付状态：" + payStatusCode + " ,支付金额：" + responseAmount + " ，应支付金额：" + db_amount);
        }
        log.debug("[威速通]-[响应支付]-3.验证第三方支付响应支付状态&验证第三方支付金额与数据库订单支付金额完成,验证结果：" + result + " ,金额验证：" + checkAmount + " ,responseAmount=" + responseAmount + " ,数据库金额：" + db_amount + ",第三方响应支付成功标志:" + payStatusCode + " ,计划成功：SUCCESS");
        return result;
    }
...
//        log.debug("[威速通]-[响应支付]-4.验证MD5签名：{}", my_result);
//        return my_result;
        Boolean boolean1 = new Boolean(signMd5);
        log.debug("[威速通]-[响应支付]-4.验证MD5签名：" + boolean1.booleanValue());
        return boolean1.booleanValue();
    }

    @Override
    protected String responseSuccess() {
        log.debug("[威速通]-[响应支付]-5.第三方支付确认收到消息返回内容：{}", RESPONSE_PAY_MSG);
