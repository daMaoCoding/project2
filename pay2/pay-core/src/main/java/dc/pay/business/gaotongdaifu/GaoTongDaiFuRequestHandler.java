...
@RequestDaifuHandler("GAOTONGDAIFU")
public final class GaoTongDaiFuRequestHandler extends DaifuRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(GaoTongDaiFuRequestHandler.class);
    ...
    protected PayEumeration.DAIFU_RESULT requestDaifuAllInOne(Map<String,String> payParam,Map<String,String> details) throws PayException {
        try {
            if(!channelWrapper.getAPI_KEY().contains("&")){
                ...
                //组装参数
                payParam.put(amount, GaoTongDaiFuUtil.Encrypt3DES(HandlerUtil.getYuan(channelWrapper.getAPI_AMOUNT()),deskey));
                payParam.put(mchtid,channelWrapper.getAPI_MEMBERID());
                payParam.put(version,"v1.0.0.0");
                payParam.put(bankaccountname,GaoTongDaiFuUtil.Encrypt3DES(channelWrapper.getAPI_CUSTOMER_NAME(),deskey));
                payParam.put(bankaccountno,GaoTongDaiFuUtil.Encrypt3DES(channelWrapper.getAPI_CUSTOMER_BANK_NUMBER(),deskey));
                payParam.put(branchname,GaoTongDaiFuUtil.Encrypt3DES(channelWrapper.getAPI_CUSTOMER_BANK_BRANCH(),deskey));
                payParam.put(bankcode,GaoTongDaiFuUtil.Encrypt3DES(channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG(),deskey));
                payParam.put(transtime,channelWrapper.getAPI_OrDER_TIME());
                payParam.put(transid, channelWrapper.getAPI_ORDER_ID());

                //生成md5
                String reqParamJsonStr = GaoTongDaiFuUtil.mapToParamByKeysSort(payParam)+"&key="+md5key;
                String pay_md5sign = GaoTongDaiFuUtil.MD5(reqParamJsonStr, "UTF-8").toLowerCase();// 32位
   ...
   // 其他情况抛异常
    ...
    protected PayEumeration.DAIFU_RESULT queryDaifuAllInOne(Map<String, String> payParam,Map<String, String> details) throws PayException {
       if(1==2) throw new PayException("[高通代付][代付][查询订单状态]该功能未完成。");
        try {
            if(!channelWrapper.getAPI_KEY().contains("&")){
    ...
    protected long queryDaifuBalanceAllInOne(Map<String,String> payParam,Map<String,String> details) throws PayException {
         throw new PayException("[高通代付][代付余额查询]第三方不支持该功能。");
    }
