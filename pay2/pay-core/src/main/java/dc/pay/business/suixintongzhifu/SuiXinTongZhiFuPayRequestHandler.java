...
@Slf4j
@RequestPayHandler("SUIXINTONGZHIFU")
public final class SuiXinTongZhiFuPayRequestHandler extends PayRequestHandler {
    //private static final Logger log = LoggerFactory.getLogger(GeFuPayRequestHandler.class);
     ...
     private static final String  fxnotifystyle = "fxnotifystyle";  //	异步返回数据的类型，默认1 返回数据为表单数据（Content-Type: multipart/form-data），2 返回post json数据。
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        ...
        if(1==1){  //HandlerUtil.isWY(channelWrapper)
            ...
            payParam.put(fxnotifystyle,"2" );


            if(HandlerUtil.isWY(channelWrapper)){
                payParam.put(fxpay,bank);
                payParam.put(fxbankcode,channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
            }

        }

        log.debug("[随鑫通支付]-[请求支付]-1.组装请求参数完成：" + JSON.toJSONString(payParam));
        return payParam;
    }
...
*/				
                ...
                if(StringUtils.isBlank(resultStr)) throw new PayException(EMPTYRESPONSE);
                ...
                if(StringUtils.isNotBlank(resultStr) && resultStr.contains("<form") && !resultStr.contains("{")){
                    ...
                    if(null!=jsonResultStr && jsonResultStr.containsKey("status") && "1".equalsIgnoreCase(jsonResultStr.getString("status"))
                        ...
                        if(HandlerUtil.isYLKJ(channelWrapper)){
                            ...
                            //【参数：fxsmstyle，默认为0（不传参数），返回一个二维码图片的链接地址；1，返回二维码的收银台，通过手机扫码支付。】 当前的接口是这样，默认情况下我们会返回一个图片的链接地址。这样能处理吗？
                            if(payurl.startsWith("data:image")){
                                payurl = QRCodeUtil.decodeByBase64( payurl);
                            }else{
                                payurl = QRCodeUtil.decodeByUrl( payurl);
                            }
             ...
             log.error("[随鑫通支付]3.发送支付请求，及获取支付请求结果出错：", e);
             throw new PayException(e.getMessage(), e);
        }
        log.debug("[随鑫通支付]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：{}" ,JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[随鑫通支付]-[请求支付]-4.处理请求响应成功：" + JSON.toJSONString(requestPayResult));
