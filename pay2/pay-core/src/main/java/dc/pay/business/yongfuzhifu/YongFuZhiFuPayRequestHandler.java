package dc.pay.business.yongfuzhifu;

/**
 * ************************
 * @author tony 3556239829
 * SpayZhiFuPayRequestHandler
 */
...
@RequestPayHandler("YONGFUZHIFU")
public final class YongFuZhiFuPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(YongFuZhiFuPayRequestHandler.class);
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        ...
        if(1==1){  //HandlerUtil.isWY(channelWrapper)
            ...
            if(HandlerUtil.isWY(channelWrapper)){
                payParam.put(pay_channelCode,BANK);
        ...
        log.debug("[永付支付]-[请求支付]-1.组装请求参数完成：" + JSON.toJSONString(payParam));
        return payParam;
    }
    ...
    protected String buildPaySign(Map<String,String> params) throws PayException {
        ...
        if(1==1 ||HandlerUtil.isWY(channelWrapper)){
            paramsStr = String.format("pay_memberid^%s&pay_orderid^%s&pay_amount^%s&pay_applydate^%s&pay_channelCode^%s&pay_bankcode^%s&pay_notifyurl^%s&key=%s",
                    params.get(pay_memberid),
                    params.get(pay_orderid),
                    params.get(pay_amount),
                    params.get(pay_applydate),
                    params.get(pay_channelCode),
                    params.get(pay_bankcode),
                    params.get(pay_notifyurl),
                    channelWrapper.getAPI_KEY());
        }else{
            paramsStr = String.format("pay_memberid^%s&pay_orderid^%s&pay_amount^%s&pay_applydate^%s&pay_channelCode^%s&pay_notifyurl^%s&key=%s",
                    params.get(pay_memberid),
                    params.get(pay_orderid),
                    params.get(pay_amount),
                    params.get(pay_applydate),
        ...
        log.debug("[永付支付]-[请求支付]-2.生成加密URL签名完成：" + JSON.toJSONString(signMd5));
        return signMd5;
    }
    ...
    protected List<Map<String, String>> sendRequestGetResult(Map<String, String> payParam, String pay_md5sign) throws PayException {
        ...
        try {
            if (HandlerUtil.isWY(channelWrapper) || HandlerUtil.isYLKJ(channelWrapper)  ) {
             ...
             log.error("[永付支付]3.发送支付请求，及获取支付请求结果出错：", e);
             throw new PayException(e.getMessage(), e);
        }
        log.debug("[永付支付]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：{}" ,JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[永付支付]-[请求支付]-4.处理请求响应成功：" + JSON.toJSONString(requestPayResult));
