package dc.pay.business.yunaifu;
...
@RequestPayHandler("YUNANFU")
public final class YunAnFuPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(YunAnFuPayRequestHandler.class);
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        ...
        log.debug("[云安付]-[请求支付]-1.组装请求参数完成：" + JSON.toJSONString(payParam));
        return payParam;
    }
    protected String buildPaySign(Map payParam) throws PayException {
        ...
        log.debug("[云安付]-[请求支付]-2.生成加密URL签名完成：" + JSON.toJSONString(pay_md5sign));
        return pay_md5sign;
    }
    protected List<Map<String, String>> sendRequestGetResult(Map<String, String> payParam, String pay_md5sign) throws PayException {
        ...
        try {
            if(HandlerUtil.isWapOrApp(channelWrapper)|| HandlerUtil.isFS(channelWrapper)){
                result.put(HTMLCONTEXT, HandlerUtil.getHtmlContent(channelWrapper.getAPI_CHANNEL_BANK_URL(),payParam).toString().toString().replaceFirst("method='post'","method='get'"));
                payResultList.add(result);
            }else{
                    Result firstPayresult = HttpUtil.get(channelWrapper.getAPI_CHANNEL_BANK_URL(), null, payParam);
                    if (firstPayresult.getBody().length() < 6 || (!firstPayresult.getBody().contains("form")  && (( !firstPayresult.getBody().toLowerCase().startsWith("weixin")&&!firstPayresult.getBody().toLowerCase().startsWith("http://")&&!firstPayresult.getBody().toLowerCase().startsWith("https://"))))) {
                        log.error("[云安付]3.发送支付请求，及获取支付请求结果：" + firstPayresult.getBody() + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
                        throw new PayException(firstPayresult.getBody());
                    }
                    if (200 == firstPayresult.getStatusCode() ) {
                            result.put(QRCONTEXT, firstPayresult.getBody());
            ...
            log.error("[云安付]3.发送支付请求，及获取支付请求结果出错：", e);
            throw new PayException(e.getMessage(), e);
        }
        log.debug("[云安付]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：" + JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[云安付]-[请求支付]-4.处理请求响应成功：" + JSON.toJSONString(requestPayResult));
