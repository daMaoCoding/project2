 ...
 * Jan 9, 2019
 */
@RequestPayHandler("YONGLIZHIFU")
public final class YongLiZhiFuPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(YongLiZhiFuPayRequestHandler.class);
...
//  private static final String  cardType       = "cardType";           
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        ...
        if (null == aPI_MEMBERID || !aPI_MEMBERID.contains("&") || aPI_MEMBERID.split("&").length != 2) {
            log.error("[永利支付]-[请求支付]-“支付通道商号”输入数据格式为【中间使用&分隔】：平台商户号 mchid&商户唯一标src_code" );
            throw new PayException("[永利支付]-[请求支付]-“支付通道商号”输入数据格式为【中间使用&分隔】：平台商户号 mchid&商户唯一标src_code" );
        }
...
//                }
        ...
        log.debug("[永利支付]-[请求支付]-1.组装请求参数完成：" + JSON.toJSONString(payParam));
        return payParam;
    }
    ...
    protected List<Map<String, String>> sendRequestGetResult(Map<String, String> payParam, String pay_md5sign) throws PayException {
        ...
        if (StringUtils.isBlank(resultStr)) {
            log.error("[永利支付]-[请求支付]-3.1.发送支付请求，及获取支付请求结果：" + resultStr + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
...
//        JSONObject resJson = JSONObject.parseObject(resultStr);
        ...
        try {
            ...
            e.printStackTrace();
            log.error("[永利支付]-[请求支付]-3.2.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
            throw new PayException(resultStr);
        }
        HashMap<String, String> result = Maps.newHashMap();
        //只取正确的值，其他情况抛出异常
        if (null != jsonObject && jsonObject.containsKey("respcd") && "0000".equalsIgnoreCase(jsonObject.getString("respcd")) && null != jsonObject.getJSONObject("data") && StringUtils.isNotBlank(jsonObject.getJSONObject("data").getString("pay_params"))) {
            result.put((HandlerUtil.isWapOrApp(channelWrapper) || HandlerUtil.isYLKJ(channelWrapper)) ?  JUMPURL : QRCONTEXT, jsonObject.getJSONObject("data").getString("pay_params"));
        }else {
            log.error("[永利支付]-[请求支付]-3.3.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
            throw new PayException(resultStr);
        }
        ArrayList<Map<String, String>> payResultList = Lists.newArrayList();
        payResultList.add(result);
        log.debug("[永利支付]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：" + JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[永利支付]-[请求支付]-4.处理请求响应成功：" + JSON.toJSONString(requestPayResult));
