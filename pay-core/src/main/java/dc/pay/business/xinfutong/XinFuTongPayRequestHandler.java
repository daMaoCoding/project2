 ...
 * May 15, 2018
 */
@RequestPayHandler("XINFUTONG")
public final class XinFuTongPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(XinFuTongPayRequestHandler.class);
...
//            	put(isApp, handlerUtil.isWY(channelWrapper) ? "web" : (handlerUtil.isWapOrApp(channelWrapper) ? "H5" : "app"));
//            	put(isApp, (channelWrapper.getAPI_CHANNEL_BANK_NAME().contains("XINFUTONG_BANK_WEBWAPAPP_ZFB_SM") || handlerUtil.isWY(channelWrapper)||handlerUtil.isYLKJ(channelWrapper)) ? "web" : (handlerUtil.isWapOrApp(channelWrapper) ? "H5" : "app"));
				put(isApp, "web");
        ...
        log.debug("[信付通]-[请求支付]-1.组装请求参数完成：" + JSON.toJSONString(payParam));
        return payParam;
    }
    ...
    protected String buildPaySign(Map<String,String> api_response_params) throws PayException {
    	...
    	try {
    		...
    		log.error("[信付通]-[请求支付]-2.生成加密URL签名出错，签名出错：{}",e.getMessage(),e);
			throw new PayException(e.getMessage(),e);
    	}
    	log.debug("[信付通]-[请求支付]-2.生成加密URL签名完成：" + JSON.toJSONString(signMd5));
    	return signMd5;
    }
...
//		if (HandlerUtil.isWY(channelWrapper) || HandlerUtil.isWapOrApp(channelWrapper) || HandlerUtil.isYLKJ(channelWrapper)) {
		if (true) {
			...
			if (StringUtils.isBlank(resultStr)) {
				log.error("[信付通]-[请求支付]-3.1.发送支付请求，获取支付请求返回值异常:返回空"+",参数："+JSON.toJSONString(payParam));
				throw new PayException("返回空"+",参数："+JSON.toJSONString(payParam));
			}
			result.put("第三方返回1",resultStr); //保存全部第三方信息，上面的拆开没必要
			if (channelWrapper.getAPI_CHANNEL_BANK_NAME().contains("XINFUTONG_BANK_WEBWAPAPP_ZFB_SM")) {
				Element formEl = Jsoup.parse(resultStr).select("body form").first();
				if (null == formEl) {
					log.error("[信付通]-[请求支付]-3.2.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
				...
				if (StringUtils.isBlank(resultStr2)) {
					log.error("[信付通]-[请求支付]-3.3.发送支付请求，获取支付请求返回值异常:返回空"+",参数："+JSON.toJSONString(secondPayParam));
					throw new PayException("返回空"+",参数："+JSON.toJSONString(secondPayParam));
				}
				result.put("第三方返回2",resultStr2); //保存全部第三方信息，上面的拆开没必要
				String[] tmps = handlerUtil.replaceBlank(resultStr2).split("text:");
				if (null == tmps || tmps.length < 1) {
					log.error("[信付通]-[请求支付]-3.4.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr2) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
					throw new PayException(resultStr2);
				}
				String urls = tmps[1];
				if (StringUtils.isBlank(urls) || !urls.contains("\'")) {
					log.error("[信付通]-[请求支付]-3.5.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr2) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
					throw new PayException(resultStr2);
				}
				result.put(QRCONTEXT, urls.split("\'")[1]);
			}else {
		...
		log.debug("[信付通]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：" + JSON.toJSONString(payResultList));
	...
	protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[信付通]-[请求支付]-4.处理请求响应成功：" + JSON.toJSONString(requestPayResult));
