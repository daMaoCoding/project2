...
@Slf4j
@ResponseDaifuHandler("GAOTONGDAIFU")
public final class GaoTongDaiFuResponseHandler extends DaifuResponseHandler {
    ...
    public String processForOrderId(Map<String, String> API_RESPONSE_PARAMS) throws PayException {
        ...
        log.debug("1.[高通代付]-[代付回调]-获取回调订单号：{}",ordernumberR);
        return ordernumberR;
    }
    ...
    protected String buildPaySign(Map<String, String> payParam, String api_key) throws PayException {
        ...
        String reqParamJsonStr = GaoTongDaiFuUtil.mapToParamByKeysSort(resParamMap)+"&key="+md5key;
        pay_md5sign = GaoTongDaiFuUtil.MD5(reqParamJsonStr, "UTF-8").toLowerCase();// 32位
        log.debug("2.[高通代付]-[代付回调]-自建签名：{}",pay_md5sign);
        return pay_md5sign;
    }
    ...
    protected boolean checkAmount(Map<String, String> api_response_params, String amountDb) throws PayException {
        ...
        try {
            responseAmount = HandlerUtil.getFen(GaoTongDaiFuUtil.Decrypt3DES(api_response_params.get(amount), deskey));
        } catch (Exception e) {
           throw new PayException("请检查deskey,解密回调金额出错。");
        }
        boolean checkAmount = amountDb.equalsIgnoreCase(responseAmount);
        if (checkAmount)   checkResult = true;
        log.debug("3.[高通代付]-[代付回调]-验证回调金额：{}",checkResult);
        return checkResult;
    }
    ...
    protected boolean checkSignMd5(Map<String, String> api_response_params, String signMd5) {
        ...
        log.debug("4.[高通代付]-[代付回调]-验证第三方签名：{}",result);
        return result;
    }
    ...
    protected String responseSuccess() {
        log.debug("5.[高通代付]-[代付回调]-响应第三方内容：{}",RESPONSE_PAY_MSG);
    ...
    protected PayEumeration.DAIFU_RESULT  resOrderStatus(Map<String, String> api_response_params) throws PayException {
        ...
        log.debug("6.[高通代付]-[代付回调]-订单状态：{}",orderStatus);
        return orderStatus;
    }
