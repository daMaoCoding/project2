package dc.pay.business.jjzhifu;
...
@RequestPayHandler("JJZHIFU")
public final class JJZhiFuPayRequestHandler extends PayRequestHandler {
        private static final Logger log = LoggerFactory.getLogger(JJZhiFuPayRequestHandler.class);
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        ...
        log.debug("[JJ支付]-[请求支付]-1.组装请求参数完成：" + JSON.toJSONString(payParam));
        return payParam;
    }
    ...
    protected String buildPaySign(Map<String,String> params) throws PayException {
        ...
        log.debug("[JJ支付]-[请求支付]-2.生成加密URL签名完成：" + JSON.toJSONString(pay_md5signA));
        return pay_md5signA;
    }
    ...
    protected List<Map<String, String>> sendRequestGetResult(Map<String, String> payParam, String pay_md5sign) throws PayException {
        ...
        String resultStr;
        try {
            if (HandlerUtil.isWY(channelWrapper) || HandlerUtil.isYLKJ(channelWrapper) || channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG().equalsIgnoreCase("904") ||  channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG().equalsIgnoreCase("903") ||  channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG().equalsIgnoreCase("913") ) { //对接时候只能使用微信，支付宝wap(返回html)
                 result.put(HTMLCONTEXT, HandlerUtil.getHtmlContent(channelWrapper.getAPI_CHANNEL_BANK_URL(),payParam).toString().replace("method='post'","method='post'"));
                 payResultList.add(result);
            }else{


                //一下内容属于解析二维码内容，目前没用，因为直接跳到第三方二维码页面，希望用户懂得截屏和导入图片，目前第三方不支持解析页面
               ...
               if(StringUtils.isNotBlank(qrContent)){
                ...
                resultStr = RestTemplateUtil.sendByRestTemplateRedirectWithSendSimpleForm(channelWrapper.getAPI_CHANNEL_BANK_URL(), payParam,HttpMethod.POST);
             ...
             log.error("[JJ支付]3.发送支付请求，及获取支付请求结果出错：", e);
             throw new PayException(e.getMessage(), e);
        }
        log.debug("[JJ支付]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：{}" ,JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[JJ支付]-[请求支付]-4.处理请求响应成功：" + JSON.toJSONString(requestPayResult));
