 ...
 */
@Slf4j
@RequestPayHandler("XINWANNENGCHONG")
public final class XinWanNengChongPayRequestHandler extends PayRequestHandler {
...
//     private static final String  sign = "sign";     // 签名=MD5(companyId_userOrderId_fee_用户密钥) 小写 参数之间用下划线连接
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        ...
        log.debug("[新万能充]-[请求支付]-1.组装请求参数完成：" + JSON.toJSONString(payParam));
        return payParam;
    }
        ...
        if (1==2 &&  HandlerUtil.isWY(channelWrapper) &&   HandlerUtil.isYLKJ(channelWrapper)  &&   HandlerUtil.isWapOrApp(channelWrapper)    ) {
            ...
            if(StringUtils.isNotBlank(resultStr) && resultStr.contains("<form") && !resultStr.contains("{")){
                ...
                if(null!=jsonResultStr && jsonResultStr.containsKey("result") && "0".equalsIgnoreCase(jsonResultStr.getString("result"))
                    ...
                    log.error("[新万能充]-[请求支付]-3.1.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
                    throw new PayException(resultStr); 
                }
            }else{ 
                log.error("[新万能充]-[请求支付]-3.2.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
                throw new PayException(EMPTYRESPONSE);
            }
        }
        log.debug("[新万能充]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：{}" ,JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[新万能充]-[请求支付]-4.处理请求响应成功：" + JSON.toJSONString(requestPayResult));
