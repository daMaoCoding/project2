...
@RequestPayHandler("MASHANGFU")
public final class MaShangFuTongPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(MaShangFuTongPayRequestHandler.class);
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        ...
        log.debug("[玛上付]-[请求支付]-1.组装请求参数完成：" + JSON.toJSONString(reqParam));
        return reqParam;
    }
       ...
       payParam.put(channelWrapper.getAPI_CHANNEL_SIGN_PARAM_NAME(), pay_md5sign);
        ...
        try {
            if (1==2 && HandlerUtil.isWY(channelWrapper) && HandlerUtil.isYLKJ(channelWrapper) && HandlerUtil.isWapOrApp(channelWrapper)    ) {
                    ...
                    if(null!=jsonResultStr && jsonResultStr.containsKey("resp")  && null!=jsonResultStr.getString("resp")){
                        ...
                        if(null!=resp && resp.containsKey("respcode") && "00".equalsIgnoreCase(resp.getString("respcode")) && resp.containsKey("formaction")  &&  StringUtils.isNotBlank(resp.getString("formaction"))){
                            ...
                            if(HandlerUtil.isWapOrApp(channelWrapper)){
                                result.put(JUMPURL,  resp.getString("formaction"));
                                payResultList.add(result);
                            }else{
             ...
             log.error("[玛上付]3.发送支付请求，及获取支付请求结果出错：", e);
             throw new PayException(e.getMessage(), e);
        }
        log.debug("[玛上付]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：{}" ,JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[玛上付]-[请求支付]-4.处理请求响应成功：" + JSON.toJSONString(requestPayResult));
