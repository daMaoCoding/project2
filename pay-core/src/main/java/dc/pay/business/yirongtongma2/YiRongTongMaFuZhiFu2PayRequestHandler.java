package dc.pay.business.yirongtongma2;
...
@RequestPayHandler("YIRONGTONGMAZHIFU2")
public final class YiRongTongMaFuZhiFu2PayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(YiRongTongMaFuZhiFu2PayRequestHandler.class);
    ...
    private static final String PARSEHTML = "parseHtml";
     ...
     private static final String  method	         = "method";     //是	Gt.online.interface
     private static final String  partner	 = "partner";            //是	商户id,由易融通码支付2分配
     private static final String  banktype	     = "banktype";       //是	银行类型，default为跳转到易融通码支付2接口进行选择支付
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        ...
        log.debug("[易融通码支付2]-[请求支付]-1.组装请求参数完成：" + JSON.toJSONString(payParam));
        return payParam;
    }
    ...
    protected String buildPaySign(Map api_response_params) throws PayException {
       // version={0}&method={1}&partner={2}&banktype={3}&paymoney={4}&ordernumber={5}&callbackurl={6}key
        ...
        log.debug("[易融通码支付2]-[请求支付]-2.生成加密URL签名完成：" + JSON.toJSONString(signMd5));
        return signMd5;
    }
    ...
    protected List<Map<String, String>> sendRequestGetResult(Map<String, String> payParam, String pay_md5sign) throws PayException {
        ...
        try {
            ...
            if(HandlerUtil.isWapOrApp(channelWrapper)|| HandlerUtil.isYLKJ(channelWrapper) ||HandlerUtil.isYLWAP(channelWrapper)) {
                ...
                if(resultStr.toLowerCase().contains("</form>") && api_channel_bank_name.contains("_WY_")){
                    ...
                    if (status.equalsIgnoreCase("1") && StringUtils.isBlank(message) && StringUtils.isNotBlank(qrurl) && !qrurl.contains("空")) {
                        ...
                        log.error("[易融通码支付2]3.发送支付请求，及获取支付请求结果：" +resultStr + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
            ...
            log.error("[易融通码支付2]3.发送支付请求，及获取支付请求结果出错：", e);
            throw new PayException(e.getMessage(), e);
        }
        log.debug("[易融通码支付2]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：" + JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[易融通码支付2]-[请求支付]-4.处理请求响应成功：" + JSON.toJSONString(requestPayResult));
