...
import java.util.*;
@RequestPayHandler("MOBAO")
public final class MoBaoPayRequestHandler extends PayRequestHandler {
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        Map<String, String> bankFlags = MoBaoPayUtil.parseBankFlag(channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
        if (!bankFlags.isEmpty() && bankFlags.size() == 3) {
            ...
            log.debug("[摩宝支付]-[请求支付]-1.组装请求参数完成：" + JSON.toJSONString(paramsMap));
            return paramsMap;
        } else {
            log.debug("[摩宝支付]-[请求支付]-1.组装请求参数出错：参数接口名，支付方式，银行代码解析为空或个数不对,订单号：" + channelWrapper.getAPI_ORDER_ID() + ",Flag:" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG() + ",解析结果：" + JSON.toJSONString(bankFlags));
            throw new PayException(SERVER_MSG.REQUEST_PAY_INFO__ERROR);
        }
    }
    protected String buildPaySign(Map payParam) throws PayException {
        String paramsStr = MoBaoPayUtil.generatePayRequest(payParam);
        String signMsg = MoBaoPayUtil.signData(paramsStr, channelWrapper.getAPI_KEY());
        log.debug("[摩宝支付]-[请求支付]-2.生成加密URL签名完成：" + JSON.toJSONString(signMsg));
        return signMsg;
    }
    protected List<Map<String, String>> sendRequestGetResult(Map<String, String> payParam, String pay_md5sign) throws PayException {
        if (null == payParam || !payParam.containsKey("choosePayType")) {
            log.error("[摩宝支付]发送请求前检查参数错误，参数中不包含支付方式choosePayType，" + JSON.toJSONString(payParam));
            throw new PayException(SERVER_MSG.REQUEST_PAY_INFO__ERROR);
        }
            ...
            try {
                ...
                if (null != secondPayParam && "/standard/payment/cashier.cgi".equalsIgnoreCase(secondPayParam.get("action"))) {
                    ...
                    if (wxqrcPayForm.size() == 1) {
                        ...
                        payUrlForManagerCGI = MoBaoPayUtil.buildWxUrlForManagerCGI(payFormMap);
                    } else if (aliqrcPayForm.size() == 1) {
                        payForm = aliqrcPayForm.first();
                        payFormMap = HandlerUtil.parseFormElement(payForm);
                        payUrlForManagerCGI = MoBaoPayUtil.buildZfbUrlForManagerCGI(payFormMap);
                    }
                    if (StringUtils.isNotBlank(payUrlForManagerCGI)) {
                        ...
                        if ("00".equalsIgnoreCase(payForManagerCGIResultJsonObj.get("respCode").toString())) {
                            ...
                            log.error("[摩宝支付]-解析扫码结果出错，第三方支付结果页面不正常，订单号：" + channelWrapper.getAPI_ORDER_ID() + ",通道名称：" + channelWrapper.getAPI_CHANNEL_BANK_NAME() + ",postUrl:" + epayUrl + ",payForManagerCGIResultJsonObj" + payForManagerCGIResultJsonObj.toJSONString());
                            throw new PayException(payForManagerCGIResultJsonObj.toJSONString());
                        }
                    } else {
                        log.error("[摩宝支付]-解析扫码结果出错，第三方支付结果页面不正常，提交地址不明确,订单号：" + channelWrapper.getAPI_ORDER_ID() + ",通道名称：" + channelWrapper.getAPI_CHANNEL_BANK_NAME() + ",postUrl:" + epayUrl + ",payForManagerCGIResultJsonObj" + payUrlForManagerCGI);
                        throw new PayException(SERVER_MSG.REQUEST_PAY_PARSE_RESULT_ERROR);
                    }
                }
            } catch (Exception e) {
                log.error("[摩宝支付]3.发送支付请求，及获取支付请求结果-获取微信||支付宝并解析出错，订单号：" + channelWrapper.getAPI_ORDER_ID() + ",通道名称：" + channelWrapper.getAPI_CHANNEL_BANK_NAME() + ",postUrl:" + epayUrl + ",payForManagerCGIResultJsonObj" + e.getMessage(), e);
        ...
        log.debug("[摩宝]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：" + JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[摩宝支付]-[请求支付]-4.处理请求响应成功：" + JSON.toJSONString(requestPayResult));
