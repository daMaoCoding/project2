package dc.pay.business.yingshengzhifu;
 ...
 * Jan 9, 2019
 */
@RequestPayHandler("YINGSHENGZHIFU")
public final class YingShengZhiFuPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(YingShengZhiFuPayRequestHandler.class);
...
//    private static final String version = "version"; // 参数列表
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        ...
        log.debug("[盈盛支付]-[请求支付]-1.组装请求参数完成：{}", JSON.toJSONString(payParam));
        return payParam;
    }
    ...
    protected String buildPaySign(Map<String, String> payParam) throws PayException {
        ...
        try {
            ...
            e.printStackTrace();
            log.error("[盈盛支付]-[请求支付]-2.生成加密URL签名出错，签名出错【本第三方，签名在sendRequestGetResult()方法里执行】：{}",e.getMessage(),e);
            throw new PayException(e.getMessage(),e);
        }
        log.debug("[盈盛支付]-[请求支付]-2.生成加密URL签名完成【本第三方，签名在sendRequestGetResult()方法里执行】：" + JSON.toJSONString(sign));
        return sign;
    }
    ...
    protected List<Map<String, String>> sendRequestGetResult(Map<String, String> payParam, String pay_md5sign) throws PayException {
        ...
        try {
            ...
            e.printStackTrace();
            log.error("[盈盛支付]-[请求支付]-3.1.生成加密URL签名出错，签名出错：{}",e.getMessage(),e);
            throw new PayException(e.getMessage(),e);
        }
        ...
        if (StringUtils.isBlank(resultStr)) {
            log.error("[盈盛支付]-[请求支付]-3.2.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
            throw new PayException(resultStr);
        }
        if (!resultStr.contains("{") || !resultStr.contains("}")) {
            log.error("[盈盛支付]-[请求支付]-3.3.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
            throw new PayException(resultStr);
        }
        ...
        try {
            ...
            log.error("[盈盛支付]-[请求支付]-3.4.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
            throw new PayException(e.getMessage(),e);
        }          
        //只取正确的值，其他情况抛出异常
...
//                String decryptedData = RsaUtil.signByPrivateKey(jsonObject.getString("data"), channelWrapper.getAPI_KEY(),"SHA1WithRSA");
                if (StringUtils.isBlank(decryptedData)) {
                    log.error("[盈盛支付]-[请求支付]-3.5.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(decryptedData) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
                    throw new PayException(decryptedData);
                }
                if (!decryptedData.contains("{") || !decryptedData.contains("}")) {
                    log.error("[盈盛支付]-[请求支付]-3.6.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(decryptedData) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
                    throw new PayException(decryptedData);
                }
                jsonObject2 = JSONObject.parseObject(decryptedData);
            } catch (Exception e) {
                e.printStackTrace();
                log.error("[盈盛支付]-[请求支付]-3.7.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
                throw new PayException(resultStr);
            }
            ...
            log.error("[盈盛支付]-[请求支付]-3.8.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
            throw new PayException(resultStr);
        }
        ArrayList<Map<String, String>> payResultList = Lists.newArrayList();
        payResultList.add(result);
        log.debug("[盈盛支付]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：{}", JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[盈盛支付]-[请求支付]-4.处理请求响应成功：" + JSON.toJSONString(requestPayResult));
