package dc.pay.business.yibaotong;
...
@ResponsePayHandler("YIBAOTONG")
public final class YiBaoTongPayResponseHandler extends PayResponseHandler {
    ...
    public String processForOrderId(Map<String, String> API_RESPONSE_PARAMS) throws PayException {
        ...
        log.debug("[亿宝通]-[响应支付]-1.获取支付通道响应信息中的订单号完成：" + ordernumberR);
        return ordernumberR;
    }
    ...
    protected String buildPaySign(Map<String, String> params, String api_key) throws PayException {
        ...
        if("RSA-S".equals(params.get(sign_type))){ // sign_type = "RSA-S"
            /**
             1)wpay_public_key，亿宝通公钥，每个商家对应一个固定的亿宝通公钥（不是使用工具生成的商户公钥merchant_public_key，不要混淆），
             即为亿宝通商家后台"支付管理"->"公钥管理"->"亿宝通公钥"里的绿色字符串内容
             2)demo提供的wpay_public_key是测试商户号123123123000的亿宝通公钥，请自行复制对应商户号的亿宝通公钥进行调整和替换	*/
            ...
            result = RsaUtil.validateSignByPublicKey(signInfo, wpay_public_key, params.get(sign));	// 验签   signInfo亿宝通返回的签名参数排序， wpay_public_key亿宝通公钥， wpaySign亿宝通返回的签名
        }

        //String signMd5 = HandlerUtil.getMD5UpperCase(paramsStr).toLowerCase();
        log.debug("[亿宝通]-[请求支付]-2.生成加密URL签名完成：" + JSON.toJSONString(result));
    ...
    protected boolean checkPayStatusAndMount(Map<String, String> api_response_params, String amount) throws PayException {
        ...
        if (checkAmount && payStatusCode.equalsIgnoreCase("SUCCESS")) {
            ...
            log.error("[亿宝通]-[响应支付]金额及状态验证错误,订单号：" + channelWrapper.getAPI_ORDER_ID() + ",第三方支付状态：" + payStatusCode + " ,支付金额：" + responseAmount + " ，应支付金额：" + amount);
        }
        log.debug("[亿宝通]-[响应支付]-3.验证第三方支付响应支付状态&验证第三方支付金额与数据库订单支付金额完成,验证结果：" + result + " ,金额验证：" + checkAmount + " ,responseAmount=" + responseAmount + " ,数据库金额：" + amount + ",第三方响应支付成功标志:" + payStatusCode + " ,计划成功：1");
        return result;
    }
    ...
    protected boolean checkSignMd5(Map<String, String> api_response_params, String signMd5) {
        ...
        log.debug("[亿宝通]-[响应支付]-4.验证MD5签名：" + signMd5Boolean.booleanValue());
        return signMd5Boolean.booleanValue();
    }

    @Override
    protected String responseSuccess() {
        log.debug("[亿宝通]-[响应支付]-5.第三方支付确认收到消息返回内容：" + RESPONSE_PAY_MSG);
