...
@RequestPayHandler("HUANYUZHIFU")
public final class HuanYuZhiFuPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(HuanYuZhiFuPayRequestHandler.class);
    ...
    private static final  String  version	     = "version";        //是	固定值3.0
     private static final String  method	         = "method";     //是
     private static final String  partner	 = "partner";            //是	商户id,由环宇支付分配
     private static final String  banktype	     = "banktype";       //是	银行类型，default为跳转到环宇支付接口进行选择支付
     ...
     private static final String  callBackUrl	 = "/respPayWeb/HUANYUZHIFU_BANK_NULL_FOR_CALLBACK/";
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        Map<String, String> payParam = new TreeMap<String, String>() {
            {
                put(version, "3.0");
                put(method, "HY.online.interface");
...
//        }
        ...
        log.debug("[环宇支付]-[请求支付]-1.组装请求参数完成：" + JSON.toJSONString(payParam));
        return payParam;
    }
    ...
    protected List<Map<String, String>> sendRequestGetResult(Map<String, String> payParam, String pay_md5sign) throws PayException {
        ...
        try {
            ...
            if (HandlerUtil.isFS(channelWrapper) || HandlerUtil.isWapOrApp(channelWrapper) ||HandlerUtil.isYLKJ(channelWrapper) ) {
                ...
                if(resultStr.toLowerCase().contains("</form>") && ( HandlerUtil.isWY(channelWrapper) ||  HandlerUtil.isWapOrApp(channelWrapper)) ){
                    ...
                    if(resultStr.contains("html") &&  !resultStr.startsWith("{") ) {  //微信扫码转到微信反扫
                        ...
                        qrurl = HandlerUtil.UrlDecode(qrurl.contains("?data=")?qrurl.substring(qrurl.indexOf("?data=")+6):( StringUtils.isBlank(qrurl)?"":qrurl ) ).toString();
                        ...
                        if (status.equalsIgnoreCase("1") && StringUtils.isBlank(message) && StringUtils.isNotBlank(qrurl) && !qrurl.contains("空")) {
                            ...
                            log.error("[环宇支付]3.发送支付请求，及获取支付请求结果：" +resultStr + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
            ...
            log.error("[环宇支付]3.发送支付请求，及获取支付请求结果出错：", e);
            throw new PayException(e.getMessage(), e);
        }
        log.debug("[环宇支付]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：" + JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[环宇支付]-[请求支付]-4.处理请求响应成功：" + JSON.toJSONString(requestPayResult));
