 ...
 * Oct 30, 2018
 */
@ResponsePayHandler("MADAO")
public final class MaDaoPayResponseHandler extends PayResponseHandler {
    private final Logger log = LoggerFactory.getLogger(getClass());

    //参数名称          参数含义                         参数长度         参数说明        顺序
    //p1_MerId          商户编号                         Max(11)         4位数字，是商户在[API支付平台]系统的唯一身份标识  请登录商户后台查看        1
    ...
    //r9_BType          交易结果返回类型                 Max(1)          为“1”： 浏览器重定向（同步通知）为“2”： 服务器点对点通讯（异步通知）        11
    ...
    //hmac              签名数据                         Max(32)         产生hmac需要两个参数，并调用相关API.  参数1：STR，列表中的参数值按照签名顺序拼接所产生的字符串，注意null要转换为“”，并确保无乱码；   参数2：商户密钥；请登录商户后台查看  各语言范例已经提供封装好了的方法用于生成此参数   如果以上两个参数有错误，则该参数必然错误，导致验签失败
...
//    private static final String key        ="key";
    ...
    public String processForOrderId(Map<String, String> API_RESPONSE_PARAMS) throws PayException {
        ...
        log.debug("[码道]-[响应支付]-1.获取支付通道响应信息中的订单号完成：{}" ,ordernumberR);
        return ordernumberR;
    }
    ...
    protected String buildPaySign(Map<String, String> api_response_params, String api_key) throws PayException {
        ...
        log.debug("[码道]-[响应支付]-2.生成加密URL签名完成：{}", JSON.toJSONString(signMd5) );
        return signMd5;
    }
    ...
    protected boolean checkPayStatusAndMount(Map<String, String> api_response_params, String db_amount) throws PayException {
        ...
        if (checkAmount && payStatusCode.equalsIgnoreCase("1")) {
            ...
            log.error("[码道]-[响应支付]金额及状态验证错误,订单号：" + channelWrapper.getAPI_ORDER_ID() + ",第三方支付状态：" + payStatusCode + " ,支付金额：" + responseAmount + " ，应支付金额：" + db_amount);
        }
        log.debug("[码道]-[响应支付]-3.验证第三方支付响应支付状态&验证第三方支付金额与数据库订单支付金额完成,验证结果：" + my_result + " ,金额验证：" + checkAmount + " ,responseAmount=" + responseAmount + " ,数据库金额：" + db_amount + ",第三方响应支付成功标志:" + payStatusCode + " ,计划成功：1");
        return my_result;
    }
    ...
    protected boolean checkSignMd5(Map<String, String> api_response_params, String signMd5) {
        ...
        log.debug("[码道]-[响应支付]-4.验证MD5签名：{}", my_result);
        return my_result;
    }

    @Override
    protected String responseSuccess() {
        log.debug("[码道]-[响应支付]-5.第三方支付确认收到消息返回内容：{}", RESPONSE_PAY_MSG);
