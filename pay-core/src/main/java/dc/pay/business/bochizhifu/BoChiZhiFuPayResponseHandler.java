 ...
 * May 20, 2019
 */
@ResponsePayHandler("BOCHIZHIFU")
public final class BoChiZhiFuPayResponseHandler extends PayResponseHandler {
...
//    private static final String key        ="key";
    ...
    public String processForOrderId(Map<String, String> API_RESPONSE_PARAMS) throws PayException {
        ...
        log.debug("[博驰支付]-[响应支付]-1.获取支付通道响应信息中的订单号完成：{}" ,ordernumberR);
        return ordernumberR;
    }
...
//        String signMd5 = HandlerUtil.getMD5UpperCase(paramsStr);
        log.debug("[博驰支付]-[响应支付]-2.生成加密URL签名完成：{}", JSON.toJSONString(result) );
    ...
    protected boolean checkPayStatusAndMount(Map<String, String> api_response_params, String db_amount) throws PayException {
        ...
        if (checkAmount && payStatusCode.equalsIgnoreCase("0000")) {
            ...
            log.error("[博驰支付]-[响应支付]金额及状态验证错误,订单号：" + channelWrapper.getAPI_ORDER_ID() + ",第三方支付状态：" + payStatusCode + " ,支付金额：" + responseAmount + " ，应支付金额：" + db_amount);
        }
        log.debug("[博驰支付]-[响应支付]-3.验证第三方支付响应支付状态&验证第三方支付金额与数据库订单支付金额完成,验证结果：" + my_result + " ,金额验证：" + checkAmount + " ,responseAmount=" + responseAmount + " ,数据库金额：" + db_amount + ",第三方响应支付成功标志:" + payStatusCode + " ,计划成功：0000");
        return my_result;
    }
...
//        log.debug("[博驰支付]-[响应支付]-4.验证MD5签名：{}", my_result);
//        return my_result;
         
         Boolean signMd5Boolean = Boolean.valueOf(signMd5);
         //boolean result = api_response_params.get(sign).equalsIgnoreCase(signMd5);
         log.debug("[博驰支付]-[响应支付]-4.验证MD5签名：" + signMd5Boolean.booleanValue());
         return signMd5Boolean.booleanValue();
    }

    @Override
    protected String responseSuccess() {
        log.debug("[博驰支付]-[响应支付]-5.第三方支付确认收到消息返回内容：{}", RESPONSE_PAY_MSG);
