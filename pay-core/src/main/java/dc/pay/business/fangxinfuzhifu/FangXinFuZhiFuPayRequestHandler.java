package dc.pay.business.fangxinfuzhifu;
 ...
 * Feb 2, 2019
 */
@RequestPayHandler("FANGXINFUZHIFU")
public final class FangXinFuZhiFuPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(FangXinFuZhiFuPayRequestHandler.class);
    ...
    //tongdao 通道编码    收银台模式：0    直联模式：ZL 是   
    private static final String tongdao                ="tongdao";
    //cashier 交易类型    cashier=1   银联WAP    cashier=2   微信扫码    cashier=3   支付宝扫码    cashier=4   QQ扫码    cashier=5   网银    cashier=6   百度钱包扫码    cashier=7   京东钱包扫码    cashier=8   微信 WAP    cashier=9   支付宝 WAP    cashier=10  QQ WAP    cashier=11  银联钱包扫码    cashier=12  京东WAP    cashier=13  百度WAP    cashier=14  银联快捷    cashier=15  微信条码    是   
    private static final String cashier                ="cashier";
    //pay_reserved1   扩展字段1   此字段在返回时按原样返回    否   
...
//    private static final String signature  ="pay_md5sign";
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        Map<String, String> payParam = new TreeMap<String, String>() {
            {
                ...
                if (handlerUtil.isWY(channelWrapper)) {
                    ...
                    put(pay_bankcode,"0");
                    put(cashier,channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
                }
        ...
        log.debug("[放心付支付]-[请求支付]-1.组装请求参数完成：{}" ,JSON.toJSONString(payParam));
        return payParam;
    }
     ...
     protected String buildPaySign(Map<String,String> api_response_params) throws PayException {
        ...
        for (int i = 0; i < paramKeys.size(); i++) {
            if (!tongdao.equals(paramKeys.get(i)) && !cashier.equals(paramKeys.get(i)) && StringUtils.isNotBlank(api_response_params.get(paramKeys.get(i)))) {
        ...
        log.debug("[放心付支付]-[请求支付]-2.生成加密URL签名完成：{}" ,JSON.toJSONString(signMd5));
        return signMd5;
    }
...
//            String resultStr = RestTemplateUtil.sendByRestTemplateRedirect(channelWrapper.getAPI_CHANNEL_BANK_URL(), payParam, String.class, HttpMethod.POST);
            ...
            //    log.error("[放心付支付]-[请求支付]-3.1.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
            //    throw new PayException(resultStr);
            //    //log.error("[放心付支付]-[请求支付]-3.1.发送支付请求，获取支付请求返回值异常:返回空,参数：{}",JSON.toJSONString(map));
            ...
            if (null != jsonObject && jsonObject.containsKey("returncode") && "00".equalsIgnoreCase(jsonObject.getString("returncode"))  && jsonObject.containsKey("txcode") && StringUtils.isNotBlank(jsonObject.getString("txcode"))) {
                ...
                log.error("[放心付支付]-[请求支付]-3.2.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
                throw new PayException(resultStr);
            }
        }
        ArrayList<Map<String, String>> payResultList = Lists.newArrayList();
        payResultList.add(result);
        log.debug("[放心付支付]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：{}",JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[放心付支付]-[请求支付]-4.处理请求响应成功：{}",JSON.toJSONString(requestPayResult));
