 ...
 * Apr 28, 2018
 */
@RequestPayHandler("XUNYOUTONG")
public final class XunYouTongPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(XunYouTongPayRequestHandler.class);

	//字段名称				类型				可否为空			最大长度			说明
	//payKey			String			否				32				商户支付Key
	//orderPrice		Float			否				12				订单金额，单位：元,保留小数点后两位，测试请在10元以上
	//outTradeNo		String			否				30				商户支付订单号（长度30以内）
	//productType		String			否				8				扫码：产品类型,如：QQ钱包T0扫码:70000103,其他详见编码表		网银：产品类型,B2C T0:50000103,B2C T1:50000101	
...
//	private static final String subPayKey	  ="subPayKey";
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        Map<String, String> payParam = new TreeMap<String, String>() {
            {
            	...
            	put(orderIp,HandlerUtil.getRandomIp(channelWrapper));
            	put(returnUrl,channelWrapper.getAPI_WEB_URL());
            	put(notifyUrl,channelWrapper.getAPI_CHANNEL_BANK_NOTIFYURL());
            }
        };
        log.debug("[迅游通]-[请求支付]-1.组装请求参数完成：" + JSON.toJSONString(payParam));
        return payParam;
    }
	 ...
	 protected String buildPaySign(Map<String,String> api_response_params) throws PayException {
        ...
        log.debug("[迅游通]-[请求支付]-2.生成加密URL签名完成：" + JSON.toJSONString(pay_md5sign));
        return pay_md5sign;
    }
	...
	protected List<Map<String, String>> sendRequestGetResult(Map<String, String> payParam, String pay_md5sign) throws PayException {
		...
		if (HandlerUtil.isWY(channelWrapper) || HandlerUtil.isYLKJ(channelWrapper)) {
			...
			if (StringUtils.isBlank(resultStr)) {
				log.error("[迅游通]-[请求支付]-3.1.发送支付请求，获取支付请求返回值异常:返回空"+",参数："+JSON.toJSONString(payParam));
				throw new PayException("返回空"+",参数："+JSON.toJSONString(payParam));
			}
			JSONObject resJson = JSONObject.parseObject(resultStr);
			if (!resJson.containsKey("resultCode") || !"0000".equals(resJson.getString("resultCode"))) {
				log.error("[迅游通]-[请求支付]-3.2.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
		...
		log.debug("[迅游通]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：" + JSON.toJSONString(payResultList));
	...
	protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[迅游通]-[请求支付]-4.处理请求响应成功：" + JSON.toJSONString(requestPayResult));
