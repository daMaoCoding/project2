package dc.pay.business.gaotong3;
 ...
 * Jun 22, 2019
 */
@RequestPayHandler("GAOTONG3")
public final class GaoTong3PayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(GaoTong3PayRequestHandler.class);
    private static final String partner = "partner";
    private static final String banktype = "banktype";
    private static final String paymoney = "paymoney";
    private static final String ordernumber = "ordernumber";
    private static final String callbackurl = "callbackurl";
    //商户用户唯一标识  memberid    N   N   当banktype为前台快捷（FRONTFASTPAY）时，该字段为必填
...
//    private static final String BARCODE = "barCode";
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        ...
        payParam.put(memberid, channelWrapper.getAPI_MEMBERID()+handlerUtil.getRandomStr(3));
        log.debug("[高通3]-[请求支付]-1.组装请求参数完成：" + JSON.toJSONString(payParam));
        return payParam;
    }
    ...
    protected String buildPaySign(Map payParam) throws PayException {
        ...
        log.debug("[高通3]-[请求支付]-2.生成加密URL签名完成：" + JSON.toJSONString(pay_md5sign));
        return pay_md5sign;
    }
...
//              log.error("[高通3]3.1.发送支付请求，获取支付请求返回值异常:返回空"+",参数："+JSON.toJSONString(payParam));
//              throw new PayException("返回空"+",参数："+JSON.toJSONString(payParam));
//          }
//          if (!resultStr.contains("img")) {
//              log.error("[高通3]3.2.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
...
//          if (1 != imgs.size()) {
//              log.error("[高通3]3.3.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
//              throw new PayException(resultStr);
//          }
//          try {
...
//          } catch (Exception e) {
//              log.error("[高通3]3.4.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
...
//      }
        ...
        log.debug("[高通3]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：" + JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[高通3]-[请求支付]-4.处理请求响应成功：" + JSON.toJSONString(requestPayResult));
