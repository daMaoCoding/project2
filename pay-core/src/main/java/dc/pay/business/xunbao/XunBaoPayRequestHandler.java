package dc.pay.business.xunbao;
 ...
 */
@RequestPayHandler("XUNBAO")
public class XunBaoPayRequestHandler extends PayRequestHandler {
    private static final Logger log =  LoggerFactory.getLogger(XunBaoPayRequestHandler.class);
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        ...
        log.debug("[讯宝支付]-[请求支付]-1.组装请求参数完成：" + JSON.toJSONString(payParam));
        return payParam;
    }
    @Override
    protected String buildPaySign(Map<String, String> payParam) throws PayException {
        ...
        log.debug("[讯宝支付]-[请求支付]-2.生成加密URL签名完成：" + JSON.toJSONString(pay_md5sign));
        return  pay_md5sign;
    }
    ...
    protected List<Map<String, String>> sendRequestGetResult(Map<String, String> payParam, String pay_md5sign) throws PayException {
        ...
        try{
            ...
            if (HandlerUtil.isWY(channelWrapper) ||HandlerUtil.isWapOrApp(channelWrapper) || HandlerUtil.isYLKJ(channelWrapper)) {
                ...
                if(bodyEl.html().contains("error:") || null== bodyEl.getElementsByTag("form").first()){
                ...
                if(StringUtils.isBlank(imgSrc)){
                    log.error("[讯宝支付]-[请求支付]-3.发送支付请求，及获取支付请求结果失败：二维码为空。"+bodyEl.select("div.divCode img ").first().toString());
                    throw new PayException("[讯宝支付]-[请求支付]-3.发送支付请求，及获取支付请求结果失败：二维码为空。"+bodyEl.select("div.divCode img ").first().toString());
                }
        ...
        log.debug("[讯宝支付]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：" + JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> result) throws PayException {
        ...
        log.debug("[讯宝支付]-[请求支付]-4.处理请求响应成功：" + JSON.toJSONString(requestPayResult));
