 ...
 * Nov 7, 2018
 */
@RequestPayHandler("KEXING")
public final class KeXingPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(KeXingPayRequestHandler.class);
...
//                put(productId,channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
        ...
        log.debug("[科星]-[请求支付]-1.组装请求参数完成：{}" ,JSON.toJSONString(payParam));
        return payParam;
    }
...
//            signMd5 = RsaUtil.signByPrivateKey2(paramsStr,channelWrapper.getAPI_KEY(),"utf-8"); // 签名
            ...
            e.printStackTrace();
            log.error("[科星]-[请求支付]-2.生成加密URL签名出错，签名出错：{}",e.getMessage(),e);
            throw new PayException(e.getMessage(),e);
        }
        log.debug("[科星]-[请求支付]-2.生成加密URL签名完成：{}" ,JSON.toJSONString(signMd5));
        return signMd5;
    }
    ...
    protected List<Map<String, String>> sendRequestGetResult(Map<String, String> payParam, String pay_md5sign) throws PayException {
        ...
        if ( HandlerUtil.isWY(channelWrapper)  ) {
            ...
            if (StringUtils.isBlank(resultStr)) {
                log.error("[科星]-[请求支付]-3.1.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
            ...
            try {
                ...
                log.error("[科星]-[请求支付]-3.3.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
                throw new PayException(resultStr);
            }
            ...
            if (null != resJson && resJson.containsKey("respCode") && "P000".equalsIgnoreCase(resJson.getString("respCode")) ) {
                if (handlerUtil.isWEBWAPAPP_SM(channelWrapper) && resJson.containsKey("payQRCode") && StringUtils.isNotBlank(resJson.getString("payQRCode"))) {
                    ...
                    log.error("[科星]-[请求支付]-3.4.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
                    throw new PayException(resultStr);
                }
            }else {
                log.error("[科星]-[请求支付]-3.4.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
        ...
        log.debug("[科星]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：{}",JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[科星]-[请求支付]-4.处理请求响应成功：{}",JSON.toJSONString(requestPayResult));
