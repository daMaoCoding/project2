 ...
 * Jun 6, 2018
 */
@ResponsePayHandler("SULONG")
public final class SuLongPayResponseHandler extends PayResponseHandler {
    private final Logger log = LoggerFactory.getLogger(getClass());

	//参数						参数名称 					类型（长度）			使用			说明
	//基本参数
	//merchant_code				商家号					String(12)			必选			商户签约时，速龙支付平台分配的唯一商家号。
	//notify_type				通知类型					String(14)			必选			取值如下：服务器后台异步通知：offline_notify
	//notify_id					通知校验ID					String(100)			必选			商家系统接收到此通知消息后，用此校验ID向速龙支付平台校验此通知的合法性，由32位数字和字母组成。例如：e722dceae317466bbf9cc5f1254b8b0a注：此版本暂不校验，但参数依旧保留。
	...
	//trade_no					速龙支付交易订单号			String(30)			必选			速龙支付交易订单号，举例：1000004817
	//trade_time				速龙支付交易订单时间			Date				必选			速龙支付交易订单时间，格式为：yyyy-MM-dd HH:mm:ss，举例：2013-12-01 12:23:34。
...
//	private static final String trade_time					="trade_time";
    ...
    public String processForOrderId(Map<String, String> API_RESPONSE_PARAMS) throws PayException {
        ...
        log.debug("[速龙]-[响应支付]-1.获取支付通道响应信息中的订单号完成：" + ordernumberR);
        return ordernumberR;
    }
    ...
    protected String buildPaySign(Map<String, String> api_response_params, String api_key) throws PayException {
        ...
        log.debug("[速龙]-[响应支付]-2.生成加密URL签名完成：" + JSON.toJSONString(result) );
    ...
    protected boolean checkPayStatusAndMount(Map<String, String> api_response_params, String db_amount) throws PayException {
        ...
        if (checkAmount && payStatusCode.equalsIgnoreCase("SUCCESS")) {
            ...
            log.error("[速龙]-[响应支付]金额及状态验证错误,订单号：" + channelWrapper.getAPI_ORDER_ID() + ",第三方支付状态：" + payStatusCode + " ,支付金额：" + responseAmount + " ，应支付金额：" + db_amount);
        }
        log.debug("[速龙]-[响应支付]-3.验证第三方支付响应支付状态&验证第三方支付金额与数据库订单支付金额完成,验证结果：" + result + " ,金额验证：" + checkAmount + " ,responseAmount=" + responseAmount + " ,数据库金额：" + db_amount + ",第三方响应支付成功标志:" + payStatusCode + " ,计划成功：SUCCESS");
        return result;
    }
    ...
    protected boolean checkSignMd5(Map<String, String> api_response_params, String signMd5) {
        ...
        log.debug("[速龙]-[响应支付]-4.验证MD5签名：" + boolean1.booleanValue());
        return boolean1.booleanValue();
    }

    @Override
    protected String responseSuccess() {
        log.debug("[速龙]-[响应支付]-5.第三方支付确认收到消息返回内容：" + RESPONSE_PAY_MSG);
