package dc.pay.business.wesutong;
 ...
 * Nov 14, 2018
 */
@RequestPayHandler("WESUTONG")
public final class WeSuTongPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(WeSuTongPayRequestHandler.class);

    //wap\扫码支付交易接口
    //参数                      参数名称                类型（长度）        使用            说明
    //基本参数
    //merchant_code             商家号                  String(12)          必填            商户签约时，支付平台分配的唯一商家号。举例：123001002003。
    //service_type              业务类型                String(10)          必选            固定值：alipay_scan 或 weixin_scan或tenpay_scan
    //notify_url                服务器异步通知地址      String(200)         必选            支付成功后，支付平台会主动通知商家系统，商家系统必须指定接收通知的地址。举例：http://www.vsdpay.com/Notify_Url.jsp
...
//    private static final String signature  ="sign";
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        Map<String, String> payParam = new TreeMap<String, String>() {
            {
                ...
                put(extra_return_param, channelWrapper.getAPI_MEMBERID());
                put(extend_param,"");
        ...
        log.debug("[威速通]-[请求支付]-1.组装请求参数完成：{}" ,JSON.toJSONString(payParam));
        return payParam;
    }
    ...
    protected String buildPaySign(Map<String, String> api_response_params) throws PayException {
        ...
        try {
            ...
            log.error("[威速通]-[请求支付]-2.生成加密URL签名出错，签名出错：{}",e.getMessage(),e);
            throw new PayException(e.getMessage(),e);
        }
        log.debug("[威速通]-[请求支付]-2.生成加密URL签名完成：{}" ,JSON.toJSONString(signMd5));
        return signMd5;
    }
    ...
    protected List<Map<String, String>> sendRequestGetResult(Map<String, String> payParam, String pay_md5sign) throws PayException {
        ...
        if (HandlerUtil.isWY(channelWrapper)) {
            ...
            if (StringUtils.isBlank(resultStr)) {
                log.error("[威速通]-[请求支付]-3.1.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
                throw new PayException(resultStr);
            }
            if (!resultStr.contains("<resp_code>SUCCESS</resp_code>")) {
                log.error("[威速通]-[请求支付]-3.2.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
                throw new PayException(resultStr);
            }
            ...
            try {
                ...
                if (null == mapBodys || !mapBodys.containsKey("result_code") || !"0".equals(mapBodys.get("result_code"))) {
                    log.error("[威速通]-[请求支付]-3.3.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
                    throw new PayException(resultStr);
                }
            } catch (Exception e) {
                e.printStackTrace();
                log.error("[威速通]-[请求支付]-3.4.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
                throw new PayException(resultStr);
            }
        ...
        log.debug("[威速通]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：" + JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[威速通]-[请求支付]-4.处理请求响应成功：{}",JSON.toJSONString(requestPayResult));
