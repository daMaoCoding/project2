package dc.pay.business.xinjiamanfu;
 ...
 * Feb 12, 2018
 */
@RequestPayHandler("XINJIAMANFU")
public final class XinJiaManFuRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(XinJiaManFuRequestHandler.class);
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        ...
        log.debug("[新加满付]-[请求支付]-1.组装请求参数完成：" + JSON.toJSONString(payParam));
        return payParam;
    }
    ...
    protected String buildPaySign(Map<String,String> api_response_params) throws PayException {
        ...
        log.debug("[新加满付]-[请求支付]-2.生成加密URL签名完成：" + JSON.toJSONString(signMd5));
        return signMd5;
    }
    ...
    protected List<Map<String, String>> sendRequestGetResult(Map<String, String> payParam, String pay_md5sign) throws PayException {
        ...
        if (handlerUtil.isWebYlKjzf(channelWrapper) || HandlerUtil.isWapOrApp(channelWrapper)) {
			...
			if (StringUtils.isBlank(resultStr)) {
				log.error("[新加满付]3.1.发送支付请求，获取支付请求返回值异常:返回空");
				throw new PayException("返回null，参数："+JSON.toJSONString(map)+"			请求地址："+channelWrapper.getAPI_CHANNEL_BANK_URL());
			}
			JSONObject jsonObject = null;
			try {
				jsonObject = JSONObject.parseObject(resultStr);
			} catch (Exception e) {
				log.error("[新加满付]3.2.发送支付请求，获取支付请求返回值异常:"+resultStr);
				throw new PayException(resultStr);
			}
			if (null == jsonObject || !jsonObject.containsKey("qrcode") || StringUtils.isBlank(jsonObject.getString("qrcode"))) {
				log.error("[新加满付]3.3.发送支付请求，获取支付请求返回值异常:"+jsonObject);
			...
			if (null == jsonObject || !jsonObject.containsKey("trans_amount") || StringUtils.isBlank(jsonObject.getString("trans_amount"))) {
				log.error("[新加满付]3.4.发送支付请求，获取支付请求返回值异常:"+jsonObject);
				throw new PayException(jsonObject.toJSONString());
			}
        ...
        log.debug("[新加满付]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：" + JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[新加满付]-[请求支付]-4.处理请求响应成功：" + JSON.toJSONString(requestPayResult));
