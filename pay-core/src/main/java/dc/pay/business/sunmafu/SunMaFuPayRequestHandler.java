 ...
 * Jul 9, 2018
 */
@RequestPayHandler("SUNMAFU")
public final class SunMaFuPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(SunMaFuPayRequestHandler.class);

    //参数名                  参数            可空      加入签名      说明
    //版本号                  version          N      Y      填写固定参数值：3.0
    //接口名称                method           N      Y      固定：Gt.online.interface
    //商户ID                  partner          N      Y      商户id,由系统分配
    //银行类型                banktype         N      Y      银行类型，具体参考附录1,
    //金额                    paymoney         N      Y      单位元（人民币）
    //商户订单号              ordernumber      N      Y      商户系统订单号，该订单号将作为接口的返回数据。该值需在商户系统内唯一，系统暂时不检查该值是否唯一
    //下行异步通知地址        callbackurl      N      Y      下行异步通知的地址，需要以http://开头且没有任何参数
    //下行同步通知地址        hrefbackurl      Y      N      下行同步通知过程的返回地址(在支付完成后接口将会跳转到的商户系统连接地址)。 注：若提交值无该参数，或者该参数值为空，则在支付完成后，接口将不会跳转到商户系统，用户将停留在接口系统提示支付成功的页面。
    //备注信息                attach           Y      N      备注信息，下行中会原样返回。若该值包含中文，请注意编码
    //MD5签名                 sign             N      N      MD5签名 sign N N N
...
//    private static final String signature  ="sign";
    ...
    protected Map<String, String> buildPayParam() throws PayException {
        ...
        log.debug("[孙码付]-[请求支付]-1.组装请求参数完成：{}" ,JSON.toJSONString(payParam));
        return payParam;
    }
    ...
    protected String buildPaySign(Map api_response_params) throws PayException {
        ...
        log.debug("[孙码付]-[请求支付]-2.生成加密URL签名完成：{}" ,JSON.toJSONString(signMd5));
        return signMd5;
    }
    ...
    protected List<Map<String, String>> sendRequestGetResult(Map<String, String> payParam, String pay_md5sign) throws PayException {
        ...
        if (HandlerUtil.isWapOrApp(channelWrapper)) {
            ...
            if (StringUtils.isBlank(resultStr)) {
                log.error("[孙码付]-[请求支付]-3.1.发送支付请求，获取支付请求返回值异常:返回空,参数：{}",JSON.toJSONString(payParam));
                throw new PayException("返回空,参数："+JSON.toJSONString(payParam));
            }
            if (!resultStr.contains("form")) {
            	log.error("[孙码付]-[请求支付]-3.2.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
            	throw new PayException(resultStr);
            }
            Document document = Jsoup.parse(resultStr);
            Elements payUrlInputs = document.select("[name=pay_url]");
            if(payUrlInputs.size()==0){
                log.error("[孙码付]-[请求支付]-3.3.发送支付请求，获取支付请求返回值异常:参数：{}",resultStr);
                throw new PayException(resultStr);
            }
            
            String pay_url = payUrlInputs.first().val();
            if(StringUtils.isBlank(pay_url)){
                log.error("[孙码付]-[请求支付]-3.4.发送支付请求，获取支付请求返回值异常:参数：{}",resultStr);
                throw new PayException(resultStr);
            }
            ...
            if (StringUtils.isBlank(resultStr2)) {
                log.error("[孙码付]-[请求支付]-3.3.发送支付请求，获取支付请求返回值异常:返回空,参数：{}",JSON.toJSONString(secondPayParam));
                throw new PayException("返回空,参数："+JSON.toJSONString(secondPayParam));
            }
            if (!resultStr2.contains("<a") && !resultStr2.contains("href")) {
            	log.error("[孙码付]-[请求支付]-3.4.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr2) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
            	throw new PayException(resultStr2);
            }
            String resultStr3 = RestTemplateUtil.sendByRestTemplateRedirect(Jsoup.parse(resultStr2).select("a").first().attr("href"), new TreeMap<>() , String.class, HttpMethod.GET).trim();
            if (StringUtils.isBlank(resultStr3)) {
            	log.error("[孙码付]-[请求支付]-3.5.发送支付请求，获取支付请求返回值异常:返回空,参数：{}",JSON.toJSONString(new TreeMap<>()));
            	throw new PayException("返回空,参数："+JSON.toJSONString(new TreeMap<>()));
            }
            result.put(QRCONTEXT, Jsoup.parse(resultStr3).select("[name=pay_url]").first().val());
            */
        }
        ArrayList<Map<String, String>> payResultList = Lists.newArrayList();
        payResultList.add(result);
        log.debug("[孙码付]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：{}",JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[孙码付]-[请求支付]-4.处理请求响应成功：{}",JSON.toJSONString(requestPayResult));
