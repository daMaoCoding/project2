 ...
 * Dec 10, 2018
 */
@RequestPayHandler("CHENGDINGQIAN")
public final class ChengDingQianPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(ChengDingQianPayRequestHandler.class);
...
*/
                   long longValue =  StrUtil.parseLong(channelWrapper.getAPI_AMOUNT()) - HandlerUtil.getRandomNumber(1,100);
                    ...
                    put(amount,  HandlerUtil.getYuan(channelWrapper.getAPI_AMOUNT()));
                    if (HandlerUtil.isYLKJ(channelWrapper) || handlerUtil.isWebWyKjzf(channelWrapper)) {
//                    if (HandlerUtil.isYLKJ(channelWrapper)) {
        ...
        log.debug("[宬鼎乾]-[请求支付]-1.组装请求参数完成：" + JSON.toJSONString(payParam));
        return payParam;
    }
    ...
    protected String buildPaySign(Map<String,String> api_response_params) throws PayException {
        ...
        log.debug("[宬鼎乾]-[请求支付]-2.生成加密URL签名完成：" + JSON.toJSONString(desEncrypt));
        return desEncrypt;
    }
    ...
    protected List<Map<String, String>> sendRequestGetResult(Map<String, String> payParam, String pay_md5sign) throws PayException {
        ...
        String resultStr = RestTemplateUtil.postStr(api_CHANNEL_BANK_URL, pay_md5sign,MediaType.ALL_VALUE.toString(),"Keep-Alive").trim();
        if (StringUtils.isBlank(resultStr)) {
            log.error("[宬鼎乾]-[请求支付]-3.1.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
...
//        JSONObject jsonObject = JSONObject.parseObject(DesUtil.desDecrypt(tmpStr,  channelWrapper.getAPI_KEY()));
        ...
        try {
            ...
            log.error("[宬鼎乾]-[请求支付]-3.2.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(jsonObject) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
        ...
        if (null == jsonObject || (jsonObject.containsKey("result")  && !"200".equals(jsonObject.getString("result")))) {
            log.error("[宬鼎乾]-[请求支付]-3.3.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(jsonObject) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
            throw new PayException(JSON.toJSONString(jsonObject));
        }
        //按不同的请求接口，向不同的属性设置值
...
//        if(handlerUtil.isWEBWAPAPP_SM(channelWrapper)) {
//            String imageValue = jsonObject.getString("codeUrl");
//            if (StringUtils.isBlank(imageValue)) {
//                log.error("[宬鼎乾]-[请求支付]-3.4.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(jsonObject) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
//                throw new PayException(JSON.toJSONString(jsonObject));
//            }
//            String qr = QRCodeUtil.decodeByUrl(imageValue);
//            if (StringUtils.isBlank(qr)) {
//                log.error("[宬鼎乾]-[请求支付]-3.5.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(jsonObject) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
//                throw new PayException(JSON.toJSONString(jsonObject));
//            }
//            // result.put(QRCONTEXT, QRCodeUtil.decodeByUrl(jsonObject.get("imageValue").toString()));  //已经有了，还去解析二维码？？
...
//        }
        ...
        //擦擦 2019/3/27 10:48:04
        //image就是我们自己装的二维码 或上游直接返回的二维码
        //擦擦 2019/3/27 10:48:09
        //code就是链接
        //10:48:20
        //擦擦 2019/3/27 10:48:20
        //二次你就用code自己分装
        //10:55:09
        //擦擦 2019/3/27 10:55:09
        //我们原则上不能二次的 具体要看你们这里能不能用
        //擦擦 2019/3/27 10:55:20
        //不能分只能直接取值了
        result.put(handlerUtil.isWEBWAPAPP_SM(channelWrapper) ? QRCONTEXT : JUMPURL, jsonObject.getString("codeUrl"));
        
        result.put("第三方返回",JSON.toJSONString(jsonObject)); //保存全部第三方信息，上面的拆开没必要
        payResultList.add(result);
        log.debug("[宬鼎乾]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：" + JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[宬鼎乾]-[请求支付]-4.处理请求响应成功：" + JSON.toJSONString(requestPayResult));
