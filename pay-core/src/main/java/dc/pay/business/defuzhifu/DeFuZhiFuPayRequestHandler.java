package dc.pay.business.defuzhifu;
 ...
 * Jun 29, 2019
 */
@RequestPayHandler("DEFUZHIFU")
public final class DeFuZhiFuPayRequestHandler extends PayRequestHandler {
    private static final Logger log = LoggerFactory.getLogger(DeFuZhiFuPayRequestHandler.class);
        ...
        Map<String, String> payParam = new TreeMap<String, String>() {
            {
                ...
                put(charset,"UTF-8");
                //defaultbank       String          C           网银代码，当支付方式为bankPay时，该值为空；支付方式为directPay时该值必传，值见银行列表
//                put(defaultbank, handlerUtil.isWebWyKjzf(channelWrapper) ? "" : channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
                ...
                if (handlerUtil.isWapOrApp(channelWrapper)) {
                    put(isApp, "H5");
                }else if (handlerUtil.isWY(channelWrapper) || handlerUtil.isWebWyKjzf(channelWrapper)) {
                    put(isApp, "web");
                }else {
                    put(isApp, "app");
                }
                ...
                put(paymentType,"1");
                put(paymethod,handlerUtil.isWebWyKjzf(channelWrapper) ? "bankPay" : "directPay");
        ...
        log.debug("[德付支付]-[请求支付]-1.组装请求参数完成：" + JSON.toJSONString(payParam));
        return payParam;
    }
    ...
    protected String buildPaySign(Map<String,String> api_response_params) throws PayException {
        ...
        log.debug("[德付支付]-[请求支付]-2.生成加密URL签名完成：" + JSON.toJSONString(signMd5));
        return signMd5;
    }
...
//                log.error("[德付支付]-[请求支付]-3.1.发送支付请求，获取支付请求返回值异常:返回空"+",参数："+JSON.toJSONString(payParam));
//                throw new PayException("返回空"+",参数："+JSON.toJSONString(payParam));
//            }
//            JSONObject resJson = JSONObject.parseObject(resultStr);
//            if (!resJson.containsKey("respCode") || !"S0001".equals(resJson.getString("respCode"))) {
//                log.error("[德付支付]-[请求支付]-3.2.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
...
//            result.put(QRCONTEXT, resJson.getString("codeUrl"));
            ...
            try {
                ...
                e.printStackTrace();
                log.error("[德付支付]-[请求支付]-3.1.发送支付请求，及获取支付请求结果：" + JSON.toJSONString(resultStr) + "订单号：" + channelWrapper.getAPI_ORDER_ID() + " ,通道：" + channelWrapper.getAPI_CHANNEL_BANK_NAME_FlAG());
                throw new PayException(resultStr);
            }
            //只取正确的值，其他情况抛出异常
        ...
        log.debug("[德付支付]-[请求支付]-3.发送支付请求，及获取支付请求结果成功：" + JSON.toJSONString(payResultList));
    ...
    protected RequestPayResult buildResult(List<Map<String, String>> resultListMap) throws PayException {
        ...
        log.debug("[德付支付]-[请求支付]-4.处理请求响应成功：" + JSON.toJSONString(requestPayResult));
