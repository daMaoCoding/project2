...
<div class="container-fluid">
    <div>
       <#-- <h1 class="text-center">响应代付流水表</h1>-->

        <form  class="form-inline"  action="${request.contextPath}/resDaifuList" method="post">
            <table class="table-bordered table-striped table-condensed" style="table-layout: fixed;width:100%;text-align: center;">
                <tr>
                    ...
                    <td style="width: 8%;">
                        <input style="width: 100%;"  type="text" class="form-control input-sm" name="resDaifuRemoteIp" value="<#if queryParam.resDaifuRemoteIp??>${queryParam.resDaifuRemoteIp}</#if>"/>
                    </td>
       ...
       <#-- <h3 class="text-success bg-info text-center"  style="margin-bottom: 0px;">-= 查询结果 =-  <#-- [<a href="${request.contextPath}/reqPayList/add">新增</a>]--></h3>
        <table class="table table-bordered table-hover table-striped table-condensed" style="table-layout: fixed;width:100%;text-align: center;">
            ...
            <tr style="background-color: #ffa07a;color:black;" class="bg-info">
                ...
                <th class="text-center" style="width: 2%;"  >验证</th>
                <th class="text-center" style="width: 3%;"  >订单状态</th>
            ...
            <tbody>
                <#list pageInfo.list as resDaifuList>
                        <#if resDaifuList.result =="SUCCESS" && resDaifuList.orderState=="SUCCESS"><tr class="success" id="${resDaifuList.id?c}"><#else> <tr id="${resDaifuList.id?c}" class="danger"></#if>
                    <#--<tr class="success">-->
                    <td>${resDaifuList_index!}</td>
                        <#if  ShowIdAndOid??>
                    <td>${resDaifuList.id!?c}</td>
                    <td>${resDaifuList.oid!}</td>
                        <#else>
                         <td>  <#if oidMaps?exists> ${oidMaps[resDaifuList.oid!]!} </#if>  </td>
                    </#if>

                    <td>${resDaifuList.orderId!}</td>
                    <#if ShowApiAmount??>  <td   class="text-right"><#if resDaifuList.amount??> ${((resDaifuList.amount?number)/100)?string(',##0.00')}<#--元--><#else> </#if></td></#if>
                    <td  class="text-left">${resDaifuList.channel!}</td>
                    <#if ShowMemberID??>
                     <td  class="text-left">${resDaifuList.channelMemberId!}</td>
                    </#if>
                    <td  class="text-left">${resDaifuList.channelCName!}</td>
                    <td>${resDaifuList.timeStmp?string('yyyy-MM-dd HH:mm:ss')}</td>
                    <td class="text-left">${resDaifuList.serverId!}</td>
                    <td class="text-left"> <#if resDaifuList??  &&  resDaifuList.resDaifuRemoteIp??>${resDaifuList.resDaifuRemoteIp!}<#else></#if></td>
                    <td>
                       <#if resDaifuList.result?? && 'SUCCESS'==resDaifuList.result >成功<#elseif resDaifuList.result?? && 'ERROR'==resDaifuList.result >失败<#else> </#if>
                    </td>

                     <td>
                      <#-- <#if resDaifuList.orderState?? && 'SUCCESS'==resDaifuList.orderState >成功<#elseif resDaifuList.orderState?? && 'ERROR'==resDaifuList.orderState >失败<#else> </#if>-->
                         ${resDaifuList.orderState!}
                      </td>


                    <td>${resDaifuList.responseDaifuParams?size} (个)</td>
                    <#if resDaifuList.resDbResult! =="ERROR"><td class="danger"><#else> <td class="success"></#if>
                    <#if resDaifuList.resDbResult?? && 'SUCCESS'==resDaifuList.resDbResult >成功<#elseif resDaifuList.resDbResult?? && 'ERROR'==resDaifuList.resDbResult >失败<#else> </#if>

                    <td> ${resDaifuList.resDbCount!}  </td>
                    <td style="text-align:center;font-size: 13px;">
                        [<a  href="${request.contextPath}/resDaifuList/view/${resDaifuList.id?c}">详情</a>]
                       <#if ShowResDaifuListDelete?? > - [<a href="${request.contextPath}/resDaifuList/delete/${resDaifuList.id?c}">删除</a>] </#if>
                       <#if resDaifuList.result?? && 'SUCCESS'==resDaifuList.result >
                           [<a onclick="bufa(this,'${resDaifuList.orderId!}','${request.contextPath}/resDaifuList/bufa/${resDaifuList.id?c}')" ref="#">补发</a>]
                       </#if>
        ...
        <nav class="text-right" aria-label="Page navigation">
            <ul class="pagination">
                <#if pageInfo.hasPreviousPage>
                    <li>
                        <a href="${request.contextPath}/resDaifuList?page=1&rows=${pageInfo.pageSize!?c}&orderId=${queryParam.orderId}&oid=${queryParam.oid}&channel=${queryParam.channel}&channelMemberId=${queryParam.channelMemberId}&result=${queryParam.result}&resDbResult=${queryParam.resDbResult}&resDaifuRemoteIp=${queryParam.resDaifuRemoteIp}&resDbCount=${queryParam.resDbCount}">首页</a>
                    </li>
                    <li>
                        <a href="${request.contextPath}/resDaifuList?page=${pageInfo.prePage!?c}&rows=${pageInfo.pageSize!?c}&orderId=${queryParam.orderId}&oid=${queryParam.oid}&channel=${queryParam.channel}&channelMemberId=${queryParam.channelMemberId}&result=${queryParam.result}&resDbResult=${queryParam.resDbResult}&resDaifuRemoteIp=${queryParam.resDaifuRemoteIp}&resDbCount=${queryParam.resDbCount}">前一页</a>
                    </li>
                </#if>
                <#list pageInfo.navigatepageNums as nav>
                    ...
                    <#if nav != pageInfo.pageNum>
                        <li>
                            <a href="${request.contextPath}/resDaifuList?page=${nav!?c}&rows=${pageInfo.pageSize!?c}&orderId=<#if queryParam.orderId??>${queryParam.orderId}</#if>&oid=<#if queryParam.oid??>${queryParam.oid}</#if>&channel=<#if queryParam.channel??>${queryParam.channel}</#if>&channelMemberId=<#if queryParam.channelMemberId??>${queryParam.channelMemberId}</#if>&result=<#if queryParam.result??>${queryParam.result}</#if>&resDbResult=<#if queryParam.resDbResult??>${queryParam.resDbResult}</#if>&resDaifuRemoteIp=<#if queryParam.resDaifuRemoteIp??>${queryParam.resDaifuRemoteIp}</#if>&resDbCount=<#if queryParam.resDbCount??>${queryParam.resDbCount}</#if>">${nav!?c}</a>
                        </li>
                    </#if>
                </#list>
                <#if pageInfo.hasNextPage>
                    <li>
                        <a href="${request.contextPath}/resDaifuList?page=${pageInfo.nextPage!?c}&rows=${pageInfo.pageSize!?c}&orderId=<#if queryParam.orderId??>${queryParam.orderId}</#if>&oid=<#if queryParam.oid??>${queryParam.oid}</#if>&channel=<#if queryParam.channel??>${queryParam.channel}</#if>&channelMemberId=<#if queryParam.channelMemberId??>${queryParam.channelMemberId}</#if>&result=<#if queryParam.result??>${queryParam.result}</#if>&resDbResult=<#if queryParam.resDbResult??>${queryParam.resDbResult}</#if>&resDaifuRemoteIp=<#if queryParam.resDaifuRemoteIp??>${queryParam.resDaifuRemoteIp}</#if>&resDbCount=<#if queryParam.resDbCount??>${queryParam.resDbCount}</#if>">下一页</a>
                    </li>
                    <li>
                        <a href="${request.contextPath}/resDaifuList?page=${pageInfo.pages!?c}&rows=${pageInfo.pageSize!?c}&orderId=<#if queryParam.orderId??>${queryParam.orderId}</#if>&oid=<#if queryParam.oid??>${queryParam.oid}</#if>&channel=<#if queryParam.channel??>${queryParam.channel}</#if>&channelMemberId=<#if queryParam.channelMemberId??>${queryParam.channelMemberId}</#if>&result=<#if queryParam.result??>${queryParam.result}</#if>&resDbResult=<#if queryParam.resDbResult??>${queryParam.resDbResult}</#if>&resDaifuRemoteIp=<#if queryParam.resDaifuRemoteIp??>${queryParam.resDaifuRemoteIp}</#if>&resDbCount=<#if queryParam.resDbCount??>${queryParam.resDbCount}</#if>">尾页</a>
                    </li>
                </#if>
                </li>
            </ul>
