...
<div class="container-fluid">
    <div>
        <h1 class="hidden text-center"><a>请求支付流水表</a></h1>
        <form  class="form-inline" action="${request.contextPath}/reqPayList" method="post">
    ...
    <h3 class="hidden text-success bg-info text-center" style="margin-bottom: 0px;">-= 查询结果 =-<#-- [<a href="${request.contextPath}/reqPayList/add">新增</a>]--></h3>
        ...
        <table class="table table-bordered table-hover table-striped table-condensed" style="table-layout: fixed;width:100%;text-align: center;">
            ...
            <tr style="background-color: #337ab7;color:white;" class="bg-info">
                ...
                <th style="width: 3%;" class="text-center">请求结果</th>
            ...
            <tbody>
                <#list pageInfo.list as reqPayList>
                <#if reqPayList.result =="SUCCESS"><tr class="success"><#else> <tr class="danger"></#if>
                <#--<tr class="success">-->
                    <td>${reqPayList_index!}</td>
                    <#if ShowIdAndOid??>
                        <td>${reqPayList.id!?c}</td>
                        <td>${reqPayList.oid!}</td>
                    <#else>
                        <td>  <#if oidMaps?exists> ${oidMaps[reqPayList.oid!]!} </#if>  </td>
                    </#if>
                    <td>${reqPayList.orderId!}</td>
                    <#if ShowApiAmount??> <td class="text-right"><#if reqPayList.amount??> ${((reqPayList.amount?number)/100)?string(',##0.00')}<#--元--><#else> </#if></td> </#if>
                    <td class="text-left">${reqPayList.channel!}</td>
                    <td class="text-right">${reqPayList.requestPayResult.requestPayChannelTime!}</td>
                    <td class="text-right">${reqPayList.requestPayResult.requestPayGetReqpayinfoTime!}</td>
                    <#if ShowMemberID??> <td  class="text-left">${reqPayList.channelMemberId!}</td> </#if>
                    <td  class="text-left">${reqPayList.channelCName!}</td>
                    <td>${reqPayList.timeStmp?string('yyyy-MM-dd HH:mm:ss')}</td>
                    <td>
                         <#if reqPayList.result?? && 'SUCCESS'==reqPayList.result >成功<#elseif reqPayList.result?? && 'ERROR'==reqPayList.result >失败<#else> </#if>
                    </td>

                <#--<td>${reqPayList.reqPayInfo!}</td>-->
                    <td>
                        <a onclick="openDig('${reqPayList.channel!}','${reqPayList.channelCName!}','${reqPayList.requestPayResult.requestPayErrorMsg!?html?js_string}')" ref="#">  ${reqPayList.requestPayResult.requestPayErrorMsg!?html} </a>
                    </td>
                    <td class="text-left">${reqPayList.serverId!}</td>

                    <td style="text-align:center;">[<a href="${request.contextPath}/reqPayList/view/${reqPayList.id?c}">详情</a>]
                    <#if reqPayListDelete?? >- [<a href="${request.contextPath}/reqPayList/delete/${reqPayList.id?c}">删除</a>] </#if>
                    </td>
                </tr>
                </#list>
            </tbody>
        ...
        <nav class="text-right" aria-label="Page navigation">
            <ul class="pagination">
                <#if pageInfo.hasPreviousPage>
                    <li>
                        <a href="${request.contextPath}/reqPayList?page=1&rows=${pageInfo.pageSize!?c}&orderId=${queryParam.orderId}&oid=${queryParam.oid}&channel=${queryParam.channel}&channelMemberId=${queryParam.channelMemberId}&result=${queryParam.result}&serverId=${queryParam.serverId}&clientIp=${queryParam.clientIp}">首页</a>
                    </li>
                    <li>
                        <a href="${request.contextPath}/reqPayList?page=${pageInfo.prePage!?c}&rows=${pageInfo.pageSize!?c}&orderId=${queryParam.orderId}&oid=${queryParam.oid}&channel=${queryParam.channel}&channelMemberId=${queryParam.channelMemberId}&result=${queryParam.result}&serverId=${queryParam.serverId}&clientIp=${queryParam.clientIp}">前一页</a>
                    </li>
                </#if>
